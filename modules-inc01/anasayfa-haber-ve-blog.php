...
<?php
$i			= 0;
$sql		= $db->query("SELECT id,baslik,url,icerik,resim FROM sayfalar WHERE site_id_555=888 AND tipi=1 AND dil='".$dil."' ORDER BY id DESC LIMIT 0,5");
while($row	= $sql->fetch(PDO::FETCH_OBJ)){
$i			= $i+1;
$link		= ($dayarlar->permalink == 'Evet') ? $row->url.'.html' : 'index.php?p=sayfa&id='.$row->id;
$icerik		= strip_tags($row->icerik);
...
$i			= 0;
$eq			= -1;
$sql		= $db->query("SELECT baslik FROM sayfalar WHERE site_id_555=888 AND tipi=1 AND dil='".$dil."' ORDER BY id DESC LIMIT 0,5");
while($row	= $sql->fetch(PDO::FETCH_OBJ)){
$i			= $i+1;
$eq			= $eq+1;
?>
