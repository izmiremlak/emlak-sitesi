...
<!-- Meta Tags -->
<title><?=$ne_ariyor;?></title>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no" />
<meta name="keywords" content="<?=($keywords != '') ? $keywords : $dayarlar->keywords;?>" />
<meta name="description" content="<?=($description != '') ? $description : $dayarlar->description;?>" /> 
...
<?if($emlak_durum != ''){?>
<a href="<?=$gvn->PermaLink($emlak_durum);?>"><?=$emlak_durum;?></a> <i class="fa fa-caret-right" aria-hidden="true"></i>
<?}?>

<?if($emlak_tipi != ''){?>
<a href="<?=$gvn->PermaLink($emlak_tipi);?>"><?=$emlak_tipi;?></a> <i class="fa fa-caret-right" aria-hidden="true"></i>
<?}?>

<?if($konut_sekli != ''){?>
<a href="<?=$gvn->PermaLink($konut_sekli);?>"><?=$konut_sekli;?></a> <i class="fa fa-caret-right" aria-hidden="true"></i>
<?}?>

<?if($konut_tipi != ''){?>
<a href="<?=$gvn->PermaLink($konut_tipi);?>"><?=$konut_tipi;?></a> <i class="fa fa-caret-right" aria-hidden="true"></i>
<?}?>

<?if($ilim->il_adi != ''){?>
<a href="<?=$ilim->slug;?>"><?=$ilim->il_adi;?></a> <i class="fa fa-caret-right" aria-hidden="true"></i>

<?if($ilcem->ilce_adi != ''){?>
<a href="<?=$ilim->slug;?>-<?=$ilcem->slug;?>"><?=$ilcem->ilce_adi;?></a> <i class="fa fa-caret-right" aria-hidden="true"></i>

<?if($mahallem->mahalle_adi != ''){?>
<a href="<?=$ilim->slug;?>-<?=$ilcem->slug;?>-<?=$mahallem->slug;?>"><?=$mahallem->mahalle_adi;?></a> <i class="fa fa-caret-right" aria-hidden="true"></i>
<?}?>
...
<?php
#$start    = microtime();
$qry			= $pagent->sql_query("SELECT t1.ilan_no,t1.id,t1.url,t1.fiyat,t1.tarih,t1.il_id,t1.ilce_id,t1.emlak_durum,t1.resim,t1.baslik,t1.pbirim,t1.metrekare,t2.id AS t2_id FROM sayfalar AS t1 LEFT JOIN dopingler_19541956 AS t2 ON t2.ilan_id=t1.id AND t2.did=4 AND t2.durum=1 AND t2.btarih>NOW() WHERE (t1.btarih>NOW() OR t2.btarih>NOW() OR EXISTS (SELECT btarih FROM dopingler_19541956 WHERE ilan_id=t1.id AND durum=1 AND btarih>NOW())) AND t1.durum=1 AND t1.ekleme=1 WWWWWWWWWWWW t1.tipi=4 ".$dahili_query." ".$dahili_order,$git,12,$execute);
try{
$query			= $qry['sql'];
...
#$end      = microtime();
#$totalt   = $start - $end;
#file_put_contents("result.txt","Start : ".$start.PHP_EOL."End: ".$end.PHP_EOL."Total: ".$totalt);
?>


<div class="altbaslik">
<h4 id="sicakfirsatlar"<?=($fonk->strlentr($ne_ariyor)>65) ? ' class="titlekisalt"' : '';?>><strong>"<? echo $fonk->kisalt2($ne_ariyor,0,76); ?>"</strong></h4>
...
$t2_id  = $row->t2_id;
$id     = $row->id;
$row_lang = $db->query("SELECT t1.id,t1.url,t1.fiyat,t1.tarih,t1.il_id,t1.ilce_id,t1.emlak_durum,t1.emlak_tipi,t1.resim,t1.baslik,t1.pbirim,t1.metrekare FROM sayfalar AS t1 WHERE t1.site_id_555=999 AND t1.ilan_no=".$row->ilan_no." AND t1.dil='".$dil."' ");
...
$isdoping5		= $db->query("SELECT id FROM dopingler_19541956 WHERE ilan_id=".$row->id." AND durum=1 AND did=5 AND btarih>NOW()")->fetch(PDO::FETCH_OBJ);
...
'baslik'		=> $row->baslik,
'fiyat'			=> ($row->fiyat != '' OR $row->fiyat != 0) ? $fiyat." ".$row->pbirim : dil("TX186"),
...
?>
  <tr<?=($rebold == 1) ? ' class="doping5"' : '';?>>
    ...
    <td>
    <a href="<?=$ilink;?>"><?=$baslik;?></a><div class="clearmob"></div>
