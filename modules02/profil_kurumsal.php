...
<div id="kfirmaprofili" class="headerbg" <?=($gayarlar->video_galeri_resim  != '') ? 'style="background-image: url(uploads/'.$gayarlar->video_galeri_resim.');"' : ''; ?>>
<div id="wrapper">
<div class="headtitle">
<h1><?=$name;?></h1>
<? if($kurumsal->unvan != ''){
...
<?if($profil->hakkinda != ''){?><a href="<?=$uyelink;?>/hakkinda" <?=($on == "hakkinda") ? 'id="kurumsalbtnaktif"' : '';?>><?=dil("TX425");?></a><?}?>
<?if($profil->turu==1 && $danismanlari>0){?><a href="<?=$uyelink;?>/danismanlar" <?=($on == "danismanlar") ? 'id="kurumsalbtnaktif"' : '';?>><?=dil("TX629");?></a><?}?>
<a href="<?=$uyelink;?>/portfoy" <?=($on == "portfoy") ? 'id="kurumsalbtnaktif"' : '';?>><?=dil("TX630");?></a>
<?if($gayarlar->anlik_sohbet==1 && $profil->tipi==0){?><a href="<?=REQUEST_URL;?>#uyemsjgonder" class="gonderbtn"><i class="fa fa-paper-plane-o" aria-hidden="true"></i> <?=dil("TX392");?></a><?}?>
</div>
</div> 
...
<div class="content" <?php if($on == "portfoy"){ ?>style="float:left;" <? }else{ ?>id="bigcontent"<? } ?>>
...
<img src="<?=$avatar;?>" alt="<?=$name;?>" title="<?=$name;?>" width="100%" height="auto">
</div> 


<div class="fprofilinfos">
<h4><strong><?=$name;?></strong></h4><h5><? echo ($il_adi != '') ? ''.$il_adi : ''; echo ($ilce_adi != '') ? ' / '.$ilce_adi : ''; echo ($mahalle_adi != '') ? ' / '.$mahalle_adi : ''; ?></h5>

<?if(($profil->sabit_telefon != '' && $profil->sabittelefond==0) || ($profil->telefon != '' && $profil->telefond==0)){?>
<span>
...
<a href="mailto:<?=$profil->email;?>">
    <i class="fa fa-envelope"></i> <?=$profil->email;?>
</a>
</span>
	<span>
<a href="https://<?=$profil->webadres;?>" target="_blank" rel="noopener noreferrer">
    <i class="fa fa-globe"></i> <?=$profil->webadres;?>
</a></span>
<div class="clear"></div>
<? if($adres != ''){ ?>
<span>
...
<? if($on == "hakkinda" && $profil->hakkinda != ''){ ?>
<!-- FİRMA HAKKINDA -->
<div class="profilfirmahakkinda">
<img style="float:right;margin:10px;" src="<?=$avatar;?>" alt="<?=$name;?>" title="<?=$name;?>" width="200" height="auto">
...
<? if($on == "danismanlar" && $profil->turu ==1 && $danismanlari>0){ ?>
<!-- FİRMA DANIŞMANLAR -->
...
<?php
$sql			= $db->query("SELECT id,kid,adi,soyadi,avatar,avatard,nick_adi FROM hesaplar WHERE site_id_555=999 AND durum=0 AND turu=2 AND kid=".$profil->id." ORDER BY id DESC");
...
if($min_fiyat != '' AND strlen($min_fiyat) < 24 AND $min_fiyat != 0){
...
if($max_fiyat != '' AND strlen($max_fiyat) < 24 AND $max_fiyat != 0){
...
if($min_metrekare != '' AND strlen($min_metrekare) < 11 AND !stristr($min_metrekare,'.')){
$filtre_count	+=1;
...
if($max_metrekare != '' AND strlen($max_metrekare) < 11 AND !stristr($max_metrekare,'.')){
$filtre_count	+=1;
...
if($min_bina_kat_sayisi != '' AND strlen($min_bina_kat_sayisi) < 11 AND !stristr($min_bina_kat_sayisi,'.')){
$filtre_count	+=1;
...
if($max_bina_kat_sayisi != '' AND strlen($max_bina_kat_sayisi) < 11 AND !stristr($max_bina_kat_sayisi,'.')){
$filtre_count	+=1;
...
$qry			= $pagent->sql_query("SELECT t1.ilan_no,t1.id,t1.url,t1.fiyat,t1.tarih,t1.il_id,t1.ilce_id,t1.emlak_durum,t1.emlak_tipi,t1.resim,t1.baslik,t1.pbirim,t1.metrekare,t2.id AS t2_id FROM sayfalar AS t1 LEFT JOIN dopingler_19541956 AS t2 ON t2.ilan_id=t1.id AND t2.did=4 AND t2.durum=1 AND t2.btarih>NOW() WHERE t1.site_id_555=999 AND (t1.btarih>NOW() OR t2.btarih>NOW() OR EXISTS
(SELECT btarih FROM dopingler_19541956 WHERE ilan_id=t1.id AND durum=1 AND btarih>NOW())) AND t1.acid IN(".$acids.") AND t1.durum=1 AND t1.tipi=4 AND t1.ekleme=1 ".$dahili_query." GROUP BY t1.ilan_no ".$dahili_order,$git,12,$execute);
try{
$query			= $qry['sql'];
...
<? include THEME_DIR."inc/advanced_search2.php"; ?>
</div>
<? } ?>
