                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="btn-toolbar" role="toolbar">
										...
										<!--div class="btn-group">
										<button type="button" class="btn btn-info waves-effect waves-light w-lg m-b-5" onclick="window.location.href='index.php?p=kategoriler';"> <i class="fa fa-table"></i> Ürün Kategorileri</button>
...
if($sil != ""){
$snc			= $db->prepare("SELECT id,video,ilan_no FROM sayfalar WHERE site_id_555=999 AND id=?");
$snc->execute(array($sil));
if($snc->rowCount()>0){
...
$snc			= $db->prepare("SELECT id,video FROM sayfalar WHERE site_id_555=999 AND id=?");
$snc->execute(array($id));
if($snc->rowCount()>0){
$snc			= $snc->fetch(PDO::FETCH_OBJ);
$db->query("DELETE FROM sayfalar WHERE site_id_555=000 AND id=".$snc->id." ");
if($snc->video != ''){
$nirde	= "/var/www/vhosts/turkiyeemlaksitesi.com.tr/turkiyeemlaksitesi.com.tr/uploads/videos/".$snc->video;
if(file_exists($nirde)){
unlink($nirde);
...
$db->query("DELETE FROM galeri_foto WHERE site_id_555=000 AND sayfa_id=".$snc->id." ");
}
}elseif($action == 'onayla'){
	try{
		$snc			= $db->prepare("SELECT id,acid,tarih,baslik FROM sayfalar WHERE site_id_555=999 AND id=?");
		$snc->execute(array($id));
	...
	if($snc->rowCount()>0){
		$snc			= $snc->fetch(PDO::FETCH_OBJ);
		$acc 			= $db->query("SELECT id,CONCAT_WS(' ',adi,soyadi) AS adsoyad,unvan,email,telefon FROM hesaplar WHERE site_id_555=999 AND id=".$snc->acid)->fetch(PDO::FETCH_OBJ);
		$db->query("UPDATE sayfalar SET durum='1' WHERE site_id_555=999 AND id=".$snc->id);
		$dnolms		= explode(" | ",dil("NOLMUS"));
		$nolms		= $dnolms[1];
		$otarih		= date("d.m.Y",strtotime($snc->tarih));
		$gtarih		= date("d.m.Y");
		$fonk->bildirim_gonder(array($acc->adsoyad,$snc->id,$snc->baslik,$nolms,date("d.m.Y H:i")),"ilan_durumu",$acc->email,$acc->telefon);
	}else{
...
?>
										...
										<tbody>
										<?php
										$sorgu		= $db->query("SELECT ilan_no,id,fiyat,baslik,gtarih,durum,acid,pbirim,url,hit FROM sayfalar XXXXXXXXXXXXXXXXXXXXXX tipi=4 AND ekleme=1 AND dil='".$dil."' ORDER BY durum ASC,id DESC LIMIT 0,500");
										while($row	= $sorgu->fetch(PDO::FETCH_OBJ)){
										$ekleyen	= $db->prepare("SELECT id,concat_ws(' ',adi,soyadi) AS adsoyad,unvan FROM hesaplar WHERE site_id_555=999 AND id=?");
										$ekleyen->execute(array($row->acid));
										...
										$isexpire     = $db->query("SELECT DISTINCT t1.id FROM sayfalar AS t1 LEFT JOIN dopingler_19541956 AS t2 ON t2.ilan_id=t1.id AND t2.durum=1 WHERE t1.site_id_555=999 AND t1.id=".$row->id." AND (t1.btarih>NOW() OR t2.btarih>NOW())");
										...
										<label for="checkbox<?=$row->id;?>"></label>
										 ...
										 <td><a href="index.php?p=uye_duzenle&id=<?=$ekleyen->id;?>"><?=$fonk->kisalt2($uye_name,0,20);?></a></td>
										 <td width="30%"><a href="../<?=$link;?>" target="_blank"><?=$fonk->kisalt2($row->baslik,0,35);?></a><br />İlan No: <strong><?=$row->ilan_no;?></strong></td>
										...
										?><span style="color:red;font-weight:bold;">Onay Bekliyor</span><?
											...
											?><span style="color:orange;font-weight:bold;">Süresi Doldu</span><?
										}elseif($row->durum == 1){
