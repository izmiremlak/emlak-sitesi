                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="btn-toolbar" role="toolbar">
                                        ...
                                        <div class="btn-group">
										<button type="button" class="btn btn-info waves-effect waves-light w-lg m-b-5" onclick="window.location.href='index.php?p=kategori_ekle';"> <i class="fa fa-plus"></i> Yeni Ekle</button>
...
?>
										...
										<tbody>
										<?php
										$sorgu		= $db->query("SELECT * FROM kategoriler_19541956 WHERE tipi=0 AND dil='".$dil."' ORDER BY id DESC LIMIT 0,500");
										while($row	= $sorgu->fetch(PDO::FETCH_OBJ)){
										if($row->ustu != 0){
										$ustu		= $db->query("SELECT * FROM kategoriler_19541956 WHERE id=".$row->ustu)->fetch(PDO::FETCH_OBJ);
										}
										?>
										<tr>
										...
										<label for="checkbox<?=$row->id;?>"></label>
										 ...
										 <td><?=$row->baslik;?></td>
										 <td><?=($dayarlar->permalink == 'Evet') ? 'kategori/'.$row->url : 'index.php?p=kategori&id='.$row->id;?></td>
										 <td><?=$row->sira; ?></td>
										 <td><?=$row->tarih;?></td>
										 <td><?=($row->ustu != 0) ? $ustu->baslik :  '<strong>Yok</strong>';?></td>
										 <td>
                                        ...
                                        <ul class="dropdown-menu" role="menu">
                                            <li><a href="index.php?p=kategori_duzenle&id=<?=$row->id;?>">Düzenle</a></li>
                                            <li><a href="index.php?p=kategoriler&sil=<?=$row->id;?>">Sil</a></li>
                                        </ul>
                                    </div>
