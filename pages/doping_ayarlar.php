                            ...
                            <div class="panel panel-default">
                                ...
                                <div class="alert alert-info" role="alert">Kullanıcıların ilan ekleme/düzenleme aşamalarında satın almak istediği doping özelliklerini ve ücretlerini tanımlayabilirsiniz.</div>
                                    ...
                                    <form role="form" class="form-horizontal"  id="forms" method="POST" action="ajax.php?p=doping_ayarlar" onsubmit="return false;" enctype="multipart/form-data">
										...
										<th>Doping Adı</th>
										<th>
										<input type="text" class="form-control" name="dzaman1a" value="<?=$dzaman1a;?>">
										<select class="form-control" name="dzaman1b">
										<?php
										foreach($periyod as $k=>$v){
										?><option value="<?=$k;?>"<?=($dzaman1b == $k) ? " selected" : '';?>><?=$v;?></option><?
										}
										?>
										</select>
										</th>
										<th>
										<input type="text" class="form-control" name="dzaman2a" value="<?=$dzaman2a;?>">
										<select class="form-control" name="dzaman2b">
										<?php
										foreach($periyod as $k=>$v){
										?><option value="<?=$k;?>"<?=($dzaman2b == $k) ? " selected" : '';?>><?=$v;?></option><?
										}
										?>
										</select>
										</th>
										<th>
										<input type="text" class="form-control" name="dzaman3a" value="<?=$dzaman3a;?>">
										<select class="form-control" name="dzaman3b">
										<?php
										foreach($periyod as $k=>$v){
										?><option value="<?=$k;?>"<?=($dzaman3b == $k) ? " selected" : '';?>><?=$v;?></option><?
										}
										?>
										</select>
										</th>
										...
										<?php
										$sql		= $db->query("SELECT * FROM doping_ayarlar_19541956 ORDER BY id ASC");
										while($row	= $sql->fetch(PDO::FETCH_OBJ)){
										?>
										<tr>
										<td><?=$row->adi;?></td>
										<td><input type="text" class="form-control" name="fiyat1[<?=$row->id;?>]" value="<?=$gvn->para_str($row->fiyat1);?>" placeholder="Tutar"></td>
										<td><input type="text" class="form-control" name="fiyat2[<?=$row->id;?>]" value="<?=$gvn->para_str($row->fiyat2);?>" placeholder="Tutar"></td>
										<td><input type="text" class="form-control" name="fiyat3[<?=$row->id;?>]" value="<?=$gvn->para_str($row->fiyat3);?>" placeholder="Tutar"></td>
