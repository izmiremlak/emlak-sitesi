...
$istatistik_sil = filter_input(INPUT_GET, 'istatistik_sil', FILTER_SANITIZE_NUMBER_INT);
...
$son30gun_ziyaret = $db->query("SELECT SUM(toplam) as toplam, SUM(tekil) as toplam2 FROM sayfa_ge_501 WHERE tarih > DATE_SUB(CURDATE(), INTERVAL 30 DAY)")->fetch()->toplam2;
$bugun_ziyaret = $db->query("SELECT toplam,tekil FROM sayfa_ge_501 WHERE tarih='".$bugun."' ")->fetch()->tekil;

// Satış istatistikleri (Bu Ay ve Bu Yıl)
$st1_ay = $db->query("SELECT SUM(tutar) AS toplam FROM dopingler_group_501 WHERE durum=1 AND tarih LIKE '%".$buay."%' ")->fetch()->toplam;
$st2_ay = $db->query("SELECT SUM(tutar) AS toplam FROM upaketler_501 WHERE durum=1 AND tarih LIKE '%".$buay."%' ")->fetch()->toplam;
$st3_ay = $db->query("SELECT SUM(tutar) AS toplam FROM onecikan_danismanlar_501 WHERE durum=1 AND tarih LIKE '%".$buay."%' ")->fetch()->toplam;
$st1_yil = $db->query("SELECT SUM(tutar) AS toplam FROM dopingler_group_501 WHERE durum=1 AND tarih LIKE '%".$buyil."%' ")->fetch()->toplam;
$st2_yil = $db->query("SELECT SUM(tutar) AS toplam FROM upaketler_501 WHERE durum=1 AND tarih LIKE '%".$buyil."%' ")->fetch()->toplam;
$st3_yil = $db->query("SELECT SUM(tutar) AS toplam FROM onecikan_danismanlar_501 WHERE durum=1 AND tarih LIKE '%".$buyil."%' ")->fetch()->toplam;
...
// Üye istatistikleri
$uyeler_ay = $db->query("SELECT COUNT(id) AS toplam FROM hesaplar WHERE (site_id_555=501 OR site_id_888=100 OR site_id_777=501501 OR site_id_699=200 OR site_id_701=501501 OR site_id_702=300) AND tipi=0 AND olusturma_tarih LIKE '%".$buay."%' ")->fetch()->toplam;
$uyeler_toplam = $db->query("SELECT COUNT(id) AS toplam FROM hesaplar WHERE (site_id_555=501 OR site_id_888=100 OR site_id_777=501501 OR site_id_699=200 OR site_id_701=501501 OR site_id_702=300) AND tipi=0")->fetch()->toplam;

// İlan istatistikleri
$ilanlar_bugun = $db->query("SELECT COUNT(id) AS toplam FROM sayfalar WHERE (site_id_555=501 OR site_id_888=100 OR site_id_777=501501 OR site_id_699=200 OR site_id_701=501501 OR site_id_702=300) AND tipi=4 AND ekleme=1 AND (durum=1 OR durum=2) AND tarih LIKE '%".$bugun."%' ")->fetch()->toplam;
$ilanlar_toplam = $db->query("SELECT COUNT(id) AS toplam FROM sayfalar WHERE (site_id_555=501 OR site_id_888=100 OR site_id_777=501501 OR site_id_699=200 OR site_id_701=501501 OR site_id_702=300) AND tipi=4 AND ekleme=1 AND (durum=1 OR durum=2)")->fetch()->toplam;

// Son siparişler
$siparisler = array('a' => array(), 'b' => array());

$query = $db->query("SELECT id,acid,adi,tutar,durum,tarih FROM upaketler_501 ORDER BY id DESC LIMIT 0,7");
while ($row = $query->fetch()) {
    ...
    $uye_adi = ($uye->rowCount() > 0) ? (($uye = $uye->fetch())->unvan != '' ? $uye->unvan : $uye->adsoyad) : "Yok";
    $urun = $row->adi." Paket Üyeliği";
    $tutar = $gvn->para_str($row->tutar)." ".dil("UYELIKP_PBIRIMI");
    $durum = ($row->durum == 0) ? '<span style="color:red;"><strong>Onay Bekliyor</strong></span>' : (($row->durum == 1) ? '<span style="color:green;"><strong>Tamamlandı</strong></span>' : (($row->durum == 2) ? '<span style="color:black;"><strong>Reddedildi</strong></span>' : ''));
...
$query = $db->query("SELECT id,acid,tutar,durum,tarih FROM dopingler_group_501 ORDER BY id DESC LIMIT 0,7");
while ($row = $query->fetch()) {
    $mictime = strtotime($row->tarih);
    $tarih = date("d.m.Y H:i", $mictime);
    $uye = $db->prepare("SELECT id,unvan,concat_ws(' ',adi,soyadi) AS adsoyad FROM hesaplar WHERE (site_id_555=501 OR site_id_888=100 OR site_id_777=501501 OR site_id_699=200 OR site_id_701=501501 OR site_id_702=300) AND id=?");
...
$query = $db->query("SELECT id,acid,tutar,durum,tarih FROM onecikan_danismanlar_501 ORDER BY id DESC LIMIT 0,7");
while ($row = $query->fetch()) {
    $mictime = strtotime($row->tarih);
    $tarih = date("d.m.Y H:i", $mictime);
    $uye = $db->prepare("SELECT id,unvan,concat_ws(' ',adi,soyadi) AS adsoyad FROM hesaplar WHERE (site_id_555=501 OR site_id_888=100 OR site_id_777=501501 OR site_id_699=200 OR site_id_701=501501 OR site_id_702=300) AND id=?");
...
while ($row = $trashimg->fetch()) {
    ...
    $db->query("DELETE FROM galeri_foto WHERE (site_id_555=501 OR site_id_888=100 OR site_id_777=501501 OR site_id_699=200 OR site_id_701=501501 OR site_id_702=300) AND id=".$row->id);
}
...
while ($snc = $trashadv->fetch()) {
    $db->query("DELETE FROM sayfalar WHERE (site_id_555=501 OR site_id_888=100 OR site_id_777=501501 OR site_id_699=200 OR site_id_701=501501 OR site_id_702=300) AND id=".$snc->id);
    if ($snc->video != '') {
    ...
    $db->query("DELETE FROM galeri_foto WHERE (site_id_555=501 OR site_id_888=100 OR site_id_777=501501 OR site_id_699=200 OR site_id_701=501501 OR site_id_702=300) AND sayfa_id=".$snc->id);
}
?>
<div class="content">
    <div class="container">
        <div class="row">
            <div class="col-sm-6 col-lg-3">
                <div class="mini-stat clearfix bx-shadow" style="background-color:#ff9800;">
                    <a class="tooltip-bottom" data-tooltip="İlan Dopingleri, Üyelik Satışları ve Danışman Öne Çıkarma satışlarınızın tutarlarıdır." href="#" style="position:absol[...]
                    <span class="mini-stat-icon"><i class="fa fa-try"></i></span> 
                    <div class="mini-stat-info text-right" style="color:white;">
                        <span style="font-size:30px;" class=""><?= sanitizeInput($satis_aylik); ?> TL</span>
                        Bu Ay Net Satış
                    </div>
                    <div class="tiles-progress">
                        <div class="m-t-20">
                            <h5 class="text-uppercase text-white m-0">Bu Yıl <span class="pull-right"><strong><?= sanitizeInput($satis_yillik); ?></strong> TL</span></h5>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-sm-6 col-lg-3">
                <div class="mini-stat clearfix bg-info bx-shadow">
                    <a class="tooltip-bottom" data-tooltip="Bireysel, Kurumsal ve Danışmanlar olarak toplam kullanıcı(üye) sayısıdır." href="#" style="position:absolute;right:40px;top:5px[...]
                    <span class="mini-stat-icon"><i class="fa fa-group" aria-hidden="true"></i></span>
                    <div class="mini-stat-info text-right">
                        <span style="font-size:30px;" class=""><?= sanitizeInput($uyeler_ay); ?></span>
                        Bu Ay Üye Sayısı
                    </div>
                    <div class="tiles-progress">
                        <div class="m-t-20">
                            <h5 class="text-uppercase text-white m-0">Toplam Üye <span class="pull-right"><strong><?= sanitizeInput($uyeler_toplam); ?></strong></span></h5>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-sm-6 col-lg-3">
                <div class="mini-stat clearfix bg-purple bx-shadow">
                    ...
                    <a class="tooltip-bottom" data-tooltip="Sayacı Sıfırla." title="Sıfırla" onclick="if(confirm('İstatistiği gerçekten silmek istiyor musunuz ?')){ window.location.href='[...]
                    <span class="mini-stat-icon"><i class="fa fa-eye" aria-hidden="true"></i></span>
                    <div class="mini-stat-info text-right">
                        <span style="font-size:30px;" class=""><?= sanitizeInput($bugun_ziyaret); ?></span>
                        Bugün Tekil Ziyaret
...
<div class="col-lg-4">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <div class="inbox-widget nicescroll mx-box" style="overflow: hidden; outline: none;" tabindex="5000">
                <?php
                $query = $db->query("SELECT id,acid,baslik,tarih,durum FROM sayfalar WHERE (site_id_555=501 OR site_id_888=100 OR site_id_777=501501 OR site_id_699=200 OR site_id_701=501501 OR site_id_702=300) AND ekleme=1 AND tipi=4 AND durum=0 ORDER BY id DESC LIMIT 0,21");
                if ($query->rowCount() > 0) {
                ...
                <a href="<?= $link; ?>">
                    <div class="inbox-item">
                        ...
                        <p class="inbox-item-text"><?= $row->baslik; ?><br><?= $durum; ?></p>
...
<div class="col-lg-4">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <div class="inbox-widget nicescroll mx-box" style="overflow: hidden; outline: none;" tabindex="5000">
                <?php
                $query = $db->query("SELECT id,concat_ws(' ',adi,soyadi) AS adsoyad,unvan,turu,olusturma_tarih FROM hesaplar WHERE site_id_555=501 AND tipi=0 AND durum=0 ORDER BY id DESC LIMIT 0,21");
                while ($row = $query->fetch(PDO::FETCH_OBJ)) {
