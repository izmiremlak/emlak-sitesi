...
$son30gun_ziyaret = $db->query("SELECT SUM(toplam) as toplam, SUM(tekil) as toplam2 FROM sayfa_ge_19541956 WHERE tarih > DATE_SUB(CURDATE(), INTERVAL 30 DAY)")->fetch()->toplam2;
$bugun_ziyaret = $db->query("SELECT toplam,tekil FROM sayfa_ge_19541956 WHERE tarih='".$bugun."' ")->fetch()->tekil;

// Satış istatistikleri (Bu Ay ve Bu Yıl)
$st1_ay = $db->query("SELECT SUM(tutar) AS toplam FROM dopingler_group_19541956 WHERE durum=1 AND tarih LIKE '%".$buay."%' ")->fetch()->toplam;
$st2_ay = $db->query("SELECT SUM(tutar) AS toplam FROM upaketler_19541956 WHERE durum=1 AND tarih LIKE '%".$buay."%' ")->fetch()->toplam;
$st3_ay = $db->query("SELECT SUM(tutar) AS toplam FROM onecikan_danismanlar_19541956 WHERE durum=1 AND tarih LIKE '%".$buay."%' ")->fetch()->toplam;
$st1_yil = $db->query("SELECT SUM(tutar) AS toplam FROM dopingler_group_19541956 WHERE durum=1 AND tarih LIKE '%".$buyil."%' ")->fetch()->toplam;
$st2_yil = $db->query("SELECT SUM(tutar) AS toplam FROM upaketler_19541956 WHERE durum=1 AND tarih LIKE '%".$buyil."%' ")->fetch()->toplam;
$st3_yil = $db->query("SELECT SUM(tutar) AS toplam FROM onecikan_danismanlar_19541956 WHERE durum=1 AND tarih LIKE '%".$buyil."%' ")->fetch()->toplam;
...
// Üye istatistikleri
$uyeler_ay = $db->query("SELECT COUNT(id) AS toplam FROM hesaplar WHERE site_id_555=999 AND tipi=0 AND olusturma_tarih LIKE '%".$buay."%' ")->fetch()->toplam;
$uyeler_toplam = $db->query("SELECT COUNT(id) AS toplam FROM hesaplar WHERE site_id_555=999 AND tipi=0")->fetch()->toplam;

// İlan istatistikleri
$ilanlar_bugun = $db->query("SELECT COUNT(id) AS toplam FROM sayfalar WHERE site_id_555=999 AND tipi=4 AND ekleme=1 AND (durum=1 OR durum=2) AND tarih LIKE '%".$bugun."%' ")->fetch()->toplam;
$ilanlar_toplam = $db->query("SELECT COUNT(id) AS toplam FROM sayfalar WHERE site_id_555=999 AND tipi=4 AND ekleme=1 AND (durum=1 OR durum=2)")->fetch()->toplam;
...
while ($row = $query->fetch()) {
    ...
    $durum = ($row->durum == 0) ? '<span style="color:red;"><strong>Onay Bekliyor</strong></span>' : (($row->durum == 1) ? '<span style="color:green;"><strong>Tamamlandı</strong></span>' : (($row->durum == 2) ? '<span style="color:black;"><strong>İptal Edildi</strong></span>' : ''));
        ...
        <div class="row">
            <div class="col-sm-6 col-lg-3">
                <div class="mini-stat clearfix bx-shadow" style="background-color:#ff9800;">
                    <a class="tooltip-bottom" data-tooltip="İlan Dopingleri, Üyelik Satışları ve Danışman Öne Çıkarma satışlarınızın tutarlarıdır." href="#" style="position:absolute;right:40px;top:5px;font-size:16px;color:white;"><i class="fa fa-info-circle"></i></a>
            ...
            <div class="col-sm-6 col-lg-3">
                <div class="mini-stat clearfix bg-info bx-shadow">
                    <a class="tooltip-bottom" data-tooltip="Bireysel, Kurumsal ve Danışmanlar olarak toplam kullanıcı(üye) sayısıdır." href="#" style="position:absolute;right:40px;top:5px;font-size:16px;color:white;"><i class="fa fa-info-circle"></i></a>
            ...
            <div class="col-sm-6 col-lg-3">
                <div class="mini-stat clearfix bg-purple bx-shadow">
                    <a class="tooltip-bottom" data-tooltip="Sitenizi ziyaret eden tekil kullanıcı sayısıdır." href="#" style="position:absolute;right:40px;top:5px;font-size:16px;color:white;"><i class="fa fa-info-circle"></i></a>
                    <a class="tooltip-bottom" data-tooltip="Sayacı Sıfırla." title="Sıfırla" onclick="if(confirm('İstatistiği gerçekten silmek istiyor musunuz ?')){ window.location.href='index.php?istatistik_sil=1'; }" href="#" style="position:absolute;right:10px;top:5px;font-size:16px;color:white;"><i class="fa fa-refresh"></i></a>
