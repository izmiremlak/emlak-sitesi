                        ...
                        <div class="panel-body">
                            ...
                            if ($hesap->tipi != 2) {
                                ...
                                if ($sil !== null) {
                                    ...
                                    if ($snc->tipi != 1) {
                                        $db->prepare("DELETE FROM hesaplar WHERE site_id_555=000 AND id=? OR kid=?")->execute([$sil, $sil]);
                                        $db->prepare("DELETE FROM sayfalar WHERE site_id_555=000 AND acid=?")->execute([$sil]);
                                        $db->prepare("DELETE FROM dopingler_19541956 WHERE acid=?")->execute([$sil]);
                                        $db->prepare("DELETE FROM dopingler_group_19541956 WHERE acid=?")->execute([$sil]);
                                        $db->prepare("DELETE FROM mesajlar_19541956 WHERE kimden=? OR kime=?")->execute([$sil, $sil]);
                                        $db->prepare("DELETE FROM mesaj_iletiler_19541956 WHERE gid=?")->execute([$sil]);
                                        $db->prepare("DELETE FROM onecikan_danismanlar_19541956 WHERE acid=?")->execute([$sil]);
                                        $db->prepare("DELETE FROM upaketler_19541956 WHERE acid=?")->execute([$sil]);
                                        $db->prepare("DELETE FROM engelli_kisiler_19541956 WHERE kim=? OR kimi=?")->execute([$sil, $sil]);
                                        $db->prepare("DELETE FROM favoriler_19541956 WHERE acid=?")->execute([$sil]);
                                        ...
                                        <td>
                                            <div class="btn-group dropdown">
                                                ...
                                                <ul class="dropdown-menu" role="menu">
                                                    ...
                                                    <li><a href="javascript:;" onclick="if(confirm('Gerçekten silmek istiyor musunuz?')){ajaxHere('ajax.php?p=danismanlar&sil=<?= sanitizeInput((string)$row->id); ?>','hidden_result');}">Sil</a></li>
                                                </ul>
                                            </div>
                                        </td>
                                    </tr>
