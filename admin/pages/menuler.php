...
if ($gvn->rakam($_GET["sil"]) != "" && $hesap->tipi != 2) {
    ...
    if (is_numeric($id)) {
        $stmt = $db->prepare("DELETE FROM menuler_19541956 WHERE id = :id");
        $stmt->execute(['id' => $id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $hesap->tipi != 2) {
    ...
    if ($baslik == "") {
        ...
        if ($duzenle == "") {
            // Ekleme işlemi
            $query = $db->prepare("INSERT INTO menuler_19541956 (dil, ustu, baslik, sira, sayfa, url, target) VALUES (:dil, :ustu, :baslik, :sira, :sayfa, :url, :target)");
            $query->execute(['dil' => $dil, 'ustu' => $ustu, 'baslik' => $baslik, 'sira' => $sira, 'sayfa' => $sayfa, 'url' => $url, 'target' => $target]);
        } else {
            // Güncelleme işlemi
            $query = $db->prepare("UPDATE menuler_19541956 SET ustu = :ustu, baslik = :baslik, sira = :sira, sayfa = :sayfa, url = :url, target = :target WHERE id = :id");
                    ...
                    <div class="panel-body">
                        ...
                        <div class="adminmenu">
                            ...
                            <form action="" method="POST" id="forms" role="form" class="form-horizontal">
                                <?php
                                if ($duzenle != '') {
                                    $srg = $db->prepare("SELECT * FROM menuler_19541956 WHERE id = :id");
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8" style="margin-top:7px;">
                                        ...
                                        <label style="float:left;margin-right:10px;" for="target_check" class="stm-checkbox-label">Aktif</label><span style="margin-left:10px;font-size:14px;margin-top:5px;">(Aktif ederseniz bağlantı tıklaması yeni sekme de açılacaktır.)</span>
                                    </div>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-9">
                                        <select name="sayfa" class="form-control">
                                            <option value="0">Yok</option>
                                            <?php
                                            $sql = $db->query("SELECT * FROM sayfalar WHERE site_id_555 = 888 AND tipi != 4 AND dil = :dil ORDER BY baslik ASC", ['dil' => $dil]);
                                            while ($row = $sql->fetch()) {
