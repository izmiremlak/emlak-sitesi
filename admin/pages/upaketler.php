...
if ($hesap->tipi != 2) {
    if ($sil !== null) {
        $db->prepare("DELETE FROM upaketler_19541956 WHERE id = ?")->execute([$sil]);
        header("Location: index.php?p=upaketler");
        exit;
    }

    if ($onayla !== null) {
        $paket = $db->prepare("SELECT * FROM upaketler_19541956 WHERE id = ?");
        ...
        $db->prepare("UPDATE upaketler_19541956 SET durum = '1' WHERE id = ?")->execute([$onayla]);
        header("Location: index.php?p=upaketler");
        exit;
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (count($idler) > 0) {
            foreach ($idler as $id) {
                ...
                if ($action === 'sil') {
                    $db->prepare("DELETE FROM upaketler_19541956 WHERE id = ?")->execute([$id]);
                    ...
                    $db->prepare("UPDATE upaketler_19541956 SET durum = '1' WHERE id = ?")->execute([$id]);
                }
            }
        }
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="btn-toolbar" role="toolbar">
                                ...
                                <div class="btn-group">
                                    <button type="button" onclick="window.location.href='index.php?p=uyelik_paketleri';" class="btn btn-danger waves-effect waves-light"><i class="fa fa-cog"></i> Üyelik Paket Özellik ve Ayarları</button>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <table class="table table-hover mails datatable">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><strong title="<?= sanitizeInput($row->sure) . " " . sanitizeInput($periyod[$row->periyod]); ?>"><?= sanitizeInput($gvn->para_str($row->tutar)); ?> <?= dil("UYELIKP_PBIRIMI"); ?></strong></td>
                                        ...
                                        <td>
                                            <div class="btn-group dropdown">
                                                ...
                                                <ul class="dropdown-menu" role="menu">
                                                    ...
                                                    <li><a href="javascript:if(confirm('Gerçekten onaylamak istiyor musunuz?')){ajaxHere('ajax.php?p=upaketler&onayla=<?= sanitizeInput((string)$row->id); ?>','hidden_result');};">Onayla</a></li>
