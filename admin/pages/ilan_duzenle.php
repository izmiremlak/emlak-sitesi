...
$ekleyen		= $db->query("SELECT id,adi,soyadi,tipi,unvan FROM hesaplar WHERE site_id_555=999 AND id=".$snc->acid)->fetch(PDO::FETCH_OBJ);
$goto			= $gvn->harf_rakam($_GET["goto"]);

if($ekleyen->unvan != ''){
$ekleyen_kim	= $ekleyen->unvan;
}else{
$ekleyen_kim	= $ekleyen->adi." ".$ekleyen->soyadi;
}
...
<div class="row">
                        <div class="col-lg-12">
                            <ul class="nav nav-tabs tabs">
                                ...
                                <span class="hidden-xs">İlan Detayları</span></a></li>
...
?>
							...
							if(isset($dilop[$o])){
								...
								$esnc	= $db->query("SELECT * FROM sayfalar WHERE site_id_555=999 AND ilan_no='".$snc->ilan_no."' AND dil='".$op->kisa_adi."' ");
								if($esnc->rowCount()>0){
							...
							<div class="tab-pane active" id="etab1"><!-- tab1 start -->
									...
									<div class="form-group">
											...
											<?php
											$sql		= $db->query("SELECT id,concat_ws(' ',adi,soyadi) AS adsoyad FROM hesaplar WHERE site_id_555=000 AND tipi=0 AND turu=2 ORDER BY id DESC");
											while($row	= $sql->fetch(PDO::FETCH_OBJ)){
											?><option value="<?=$row->id;?>" <?=($row->id == $snc->danisman_id) ? 'selected' : '';?>><?=$row->adsoyad;?></option><?
											}
                    ...
                    $isyeri = $parc[1];
										...
										<div class="form-group" id="bina_yasi_con">
                                            ...
                                            <div class="col-sm-9">
											<input type="text" class="form-control" id="bina_yasi" name="bina_yasi" value="<?=$snc->bina_yasi;?>" >
                                        </div>
                                        </div>
										...
										<div class="form-group" id="bina_kat_sayisi_con">
                                            ...
                                            <div class="col-sm-9">
											<input type="text" class="form-control" id="bina_kat_sayisi" name="bina_kat_sayisi" value="<?=$snc->bina_kat_sayisi;?>" >
                                        </div>
                                        </div>
										...
										<div class="form-group" id="banyo_sayisi_con">
                                            ...
                                            <div class="col-sm-9">
											<input type="text" class="form-control" id="banyo_sayisi" name="banyo_sayisi" value="<?=$snc->banyo_sayisi;?>" >
                                        </div>
                                        </div>
										...
										<option value="0" <?=($snc->esyali == 0 AND $snc->esyali != '') ? 'selected' : '';?>>Hayır</option>
										</select>
                                        </div>
                                        </div>
										...
										<option value="0" <?=($snc->site_ici == 0 AND $snc->site_ici != '') ? 'selected' : '';?>>Hayır</option>
										</select>
                                        </div>
                                        </div>
										...
										<div class="form-group" id="aidat_con">
                                            ...
                                            <div class="col-sm-9">
											<input type="text" class="form-control" name="aidat" value="<?=$aidat;?>" >
                                        </div>
                                        </div>
                                        ...
                                        <div class="col-sm-9">
                                        <input type="text" class="form-control" name="metrekare_fiyat" value="<?=$snc->metrekare_fiyat;?>" >
                                        </div>
                                        </div>
                                        ...
                                        <div class="col-sm-9">
                                        <input type="text" class="form-control" name="ada_no" value="<?=$snc->ada_no;?>" >
                                        </div>
                                        </div>
                                        ...
                                        <div class="col-sm-9">
                                        <input type="text" class="form-control" name="parsel_no" value="<?=$snc->parsel_no;?>" >
                                        </div>
                                        </div>
                                        ...
                                        <div class="col-sm-9">
                                        <input type="text" class="form-control" name="pafta_no" value="<?=$snc->pafta_no;?>" >
                                        </div>
                                        </div>
										...
										?>
										<input type="text" class="form-control" name="kaks_emsal" value="<?=$snc->kaks_emsal;?>" >
										<?php } ?>
                                        </div>
                                        </div>
										...
										?>
										<input type="text" class="form-control" name="gabari" value="<?=$snc->gabari;?>" >
										<?php } ?>
                                        </div>
                                        </div>
									  ...
									  <option <?=($snc->katk == $fonk->get_lang($snc->dil,"TX168")) ? "selected" : '';?>><?=$fonk->get_lang($snc->dil,"TX168");?></option>
										...
										?>
										  ...
										  <?php
										  foreach($exp as $row){
										  ?><option <?=($row == $snc->kimden) ? "selected" : '';?>><?=$row;?></option><?
										  }
										  ?>
										  </select>
										  </div>
										...
										<?php
										foreach($exp as $row){
										?><option <?=($row == $snc->yetkis) ? "selected" : '';?>><?=$row;?></option><?
										}
										?>
										</select>
										</div>
...
<div class="form-group" style="text-align: center;">
    ...
    <div style="margin-top: 10px;">
        ...
        <input type="hidden" id="site_id_335" name="site_id_335" value="<?=$snc->site_id_334=335;?>">
    </div>
    ...
    <div style="margin-top: 10px;">
        ...
        <input type="hidden" id="site_id_334" name="site_id_334" value="<?=$snc->site_id_334=334;?>">
    </div>
    ...
    <div style="margin-top: 10px;">
        ...
        <input type="hidden" id="site_id_306" name="site_id_306" value="<?=$snc->site_id_306=306;?>">
    </div>
</div>
</table>
...
?>
<span><label style="font-weight:normal;"><input name="cephe_ozellikler[]" value="<?=$val;?>" <?=$checked;?> type="checkbox"> <?=$val;?></label></span>
...
$checked	= (in_array($val,$ielm)) ? 'checked' : '';
?>
<span><label style="font-weight:normal;"><input name="ic_ozellikler[]" type="checkbox" <?=$checked;?> value="<?=$val;?>"><?=$val;?></label></span>
<?
}
?>
</div>
...
$checked	= (in_array($val,$ielm)) ? 'checked' : '';
?>
<span><label style="font-weight:normal;"><input name="dis_ozellikler[]" type="checkbox" <?=$checked;?> value="<?=$val;?>"><?=$val;?></label></span>
<?
}
?>
</div>
...
$checked	= (in_array($val,$ielm)) ? 'checked' : '';
?>
<span><label style="font-weight:normal;"><input name="altyapi_ozellikler[]" type="checkbox" <?=$checked;?> value="<?=$val;?>"><?=$val;?></label></span>
<?
}
?>
</div>
...
$checked	= (in_array($val,$ielm)) ? 'checked' : '';
?>
<span><label style="font-weight:normal;"><input name="konum_ozellikler[]" type="checkbox" <?=$checked;?> value="<?=$val;?>"><?=$val;?></label></span>
<?
}
?>
</div>
...
$checked	= (in_array($val,$ielm)) ? 'checked' : '';
?>
<span><label style="font-weight:normal;"><input name="genel_ozellikler[]" type="checkbox" <?=$checked;?> value="<?=$val;?>"><?=$val;?></label></span>
<?
}
?>
</div>
...
$checked	= (in_array($val,$ielm)) ? 'checked' : '';
?>
<span><label style="font-weight:normal;"><input name="manzara_ozellikler[]" type="checkbox" <?=$checked;?> value="<?=$val;?>"><?=$val;?></label></span>
<?
}
?>
</div>
...
<?if($multict>0 && $snc->id == $multif->id){?>
					 <div class="tab-pane<?=($goto == 'photos') ? " active " : '';?>" id="tab2">
                      ...
                      <div class="alert alert-info" role="alert"><strong style="color:red;">İlan Fotoğraflarının Uzantısı .jpeg Olmalıdır.</strong><br><strong style="color:red;">İlan fotoğraflarını 600x800 pixel veya 900x1200 pixel olarak ve Yatay çekerseniz, Fotoğraflar sitenizde daha güzel gözükür</strong><br>Sistem, yüklenen fotoğrafları filtreden geçirerek otomatik olarak tekrar boyutlandırmaktadır.<br>Yükleme yaptıktan sonra kapak fotoğrafını değiştirebilir ve Fotoğrafların yerlerini, kaydırarak sıralayabilirsiniz.</div>
...
$sql		= $db->query("SELECT * FROM galeri_foto WHERE site_id_555=999 AND sayfa_id=".$snc->id." AND dil='".$dil."' ORDER BY sira ASC");
$i			= 0;
while($row	= $sql->fetch(PDO::FETCH_OBJ)){
$i			+=1;
?>
...
<div align="center">
<video width="70%" height="500" controls><source src="/var/www/vhosts/turkiyeemlaksitesi.com.tr/turkiyeemlaksitesi.com.tr/uploads/videos/<?=$snc->video;?>" type="video/mp4"><?=$fonk->get_lang($snc->dil,"VIDEO_SUPPORT");?></video>
  </div>
  <div class="clear"></div>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	?>
	<? if($isdoping->sure == 100 && $isdoping->periyod == "yillik"){ ?>
...
if(tipi == "<?=$isyeri;?>"){
  ...
  ?><option value=''><?=$fonk->get_lang($snc->dil,"TX57");?></option><?
  $parc		= explode("<+>",$knttipi);
  foreach($parc as $val){
  ?><option><?=$val;?></option><?
  }
