...
$snc		= $db->prepare("SELECT * FROM sayfalar WHERE site_id_555=000 AND id=:ids");
...
$resim			= $fonk->resim_yukle(true,'file',$randnm,'/var/www/vhosts/turkiyeemlaksitesi.com.tr/turkiyeemlaksitesi.com.tr/uploads',$gorsel_boyutlari['foto_galeri']['thumb_x'],$gorsel_boyutlari['foto_galeri']['thumb_y'],true,$watermark,true);
$resim			= $fonk->resim_yukle(false,'file',$randnm,'/var/www/vhosts/turkiyeemlaksitesi.com.tr/turkiyeemlaksitesi.com.tr/uploads',$gorsel_boyutlari['foto_galeri']['orjin_x'],$gorsel_boyutlari['foto_galeri']['orjin_y'],true,$watermark,true);
...
if($snc->resim == '' || $snc->resim == 'default_ilan_resim.jpg'){
    $db->query("UPDATE sayfalar SET resim='".$resim."' WHERE site_id_555=999 AND id=".$snc->id);
}
...
$acc			= $db->query("SELECT id,concat_ws(' ',adi,soyadi) AS adsoyad,unvan,telefon,email FROM hesaplar WHERE site_id_555=999 AND id=".$snc->acid)->fetch(PDO::FETCH_OBJ);
...
$ulke_id		= ($ulke_id==0) ? $db->query("SELECT id FROM ulkeler_19541956 ORDER BY id DESC LIMIT 0,1")->fetch(PDO::FETCH_OBJ)->id : $ulke_id;


// Ülkeyi kontrol ediyorum....
$ulkekontrol		= $db->prepare("SELECT * FROM ulkeler_19541956 WHERE id=?");
...
if($danisman_id != 0){
$danisman	= $db->query("SELECT id FROM danismanlar_19541956 WHERE id=".$danisman_id);
...
$hesapp			= $acc;
$fonk->bildirim_gonder(array($adsoyad,$snc->id,$snc->baslik,$nolms,date("d.m.Y H:i")),"ilan_durumu",$hesapp->email,$hesapp->telefon);

} // eğer durumda bir değişiklik varsa


$tabs					= $_POST["tabs"];
if(!empty($tabs)){
	foreach($tabs AS $key=>$val){
		$dilop 	= $db->prepare("SELECT * FROM diller_19541956 WHERE kisa_adi=?");
		...
		if($dilop->rowCount()>0 && $fonk->bosluk_kontrol($val['baslik'])==false && ($multi->rowCount()>0 && $snc->id == $multif->id)){
			...
			if($pos3==false){$pos3_1=array_search($konut_tipi,$d_knt_tipi2);}
			...
			$kontrol	= $db->prepare("SELECT id,ilan_no FROM sayfalar WHERE site_id_555=999 AND ilan_no=? AND dil=?");
			...
			if($kontrol->rowCount()>0){ // var ise
				...
				try {
 				...
 				$prepare 	= "INSERT INTO sayfalar SET site_id_555=555,site_id_450=450,site_id_444=444,site_id_333=333,site_id_222=222,site_id_111=111,ekleme=?,baslik=?,fiyat=?,ilan_no=?,emlak_durum=?,emlak_tipi=?,ulke_id=?,il_id=?,ilce_id=?,mahalle_id=?,konut_tipi=?,konut_sekli=?,bulundugu_kat=?,metrekare=?,brut_metrekare=?,yapi_durum=?,oda_sayisi=?,bina_yasi=?,bina_kat_sayisi=?,isitma=?,banyo_sayisi=?,esyali=?,kullanim_durum=?,site_ici=?,kimden=?,yetkis=?,yetki_bilgisi=?,site_id_888=?,site_id_777=?,site_id_699=?,site_id_700=?,site_id_701=?,site_id_702=?,site_id_661=?,site_id_662=?,site_id_663=?,site_id_664=?,site_id_665=?,site_id_666=?,site_id_667=?,site_id_668=?,site_id_669=?,site_id_335=?,site_id_334=?,site_id_306=?,aidat=?,cephe_ozellikler=?,ic_ozellikler=?,dis_ozellikler=?,maps=?,resim=?,acid=?,tipi=?,dil=?,url=?,tarih=?,gtarih=?,btarih=?,icerik=?,title=?,keywords=?,description=?,resim2=?,durum=?,pbirim=?,metrekare_fiyat=?,ada_no=?,parsel_no=?,pafta_no=?,kaks_emsal=?,gabari=?,tapu_durumu=?,katk=?,krediu=?,takas=?,altyapi_ozellikler=?,konum_ozellikler=?,genel_ozellikler=?,manzara_ozellikler=?,notu=?,danisman_id=?";
