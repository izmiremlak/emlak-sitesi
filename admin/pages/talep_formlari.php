...
if ($hesap->tipi != 2) {
    if ($okundu !== null) {
        $db->prepare("UPDATE mail_19541956 SET durum = '1' WHERE id = ?")->execute([$okundu]);
        header("Location: index.php?p=talep_formlari");
        exit;
    } elseif ($okunmadi !== null) {
        $db->prepare("UPDATE mail_19541956 SET durum = '0' WHERE id = ?")->execute([$okunmadi]);
        header("Location: index.php?p=talep_formlari");
        exit;
    } elseif ($sil !== null) {
        $db->prepare("DELETE FROM mail_19541956 WHERE id = ?")->execute([$sil]);
        header("Location: index.php?p=talep_formlari");
        exit;
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (count($idler) > 0) {
            foreach ($idler as $id) {
                ...
                if ($action === 'okundu') {
                    $db->prepare("UPDATE mail_19541956 SET durum = '1' WHERE id = ?")->execute([$id]);
                } elseif ($action === 'okunmadi') {
                    $db->prepare("UPDATE mail_19541956 SET durum = '0' WHERE id = ?")->execute([$id]);
                } elseif ($action === 'sil') {
                    $db->prepare("DELETE FROM mail_19541956 WHERE id = ?")->execute([$id]);
                }
            }
        }
