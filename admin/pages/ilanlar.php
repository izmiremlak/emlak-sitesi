...
if ($sil !== null) {
    ...
    if ($snc->rowCount() > 0) {
        ...
        $multif = $multi->fetch();
        $multidids = $db->query("SELECT GROUP_CONCAT(id SEPARATOR ',') AS ids FROM sayfalar WHERE site_id_555=999 AND ilan_no=".$snc->ilan_no)->fetch()->ids;
    ...
    if (count($idler) > 0) {
        foreach ($idler as $id) {
            ...
            if ($action == 'sil') {
                ...
                if ($snc->rowCount() > 0) {
                    $snc = $snc->fetch();
                    $acc = $db->query("SELECT id,CONCAT_WS(' ',adi,soyadi) AS adsoyad,unvan,email,telefon FROM hesaplar WHERE site_id_555=999 AND id=".$snc->acid)->fetch();
            ...
            <div class="panel panel-pink panel-color">
                ...
                <div id="collapseOne-2" class="panel-collapse collapse" aria-expanded="true">
                    <div class="panel-body">
                        Bu alanda, tarafınızdan eklenmiş ilanlar ile üyelerin ekledikleri ilanları görebilirsiniz. Ayrıca üyeler her ilan ekleme ve güncelleme işlemlerinde bildirim alırlar.
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="panel panel-default m-t-20">
                        <div class="panel-body">
                            <table class="table table-hover mails datatable">
                                ...
                                <tbody>
                                    <?php
                                    $sorgu = $db->query("SELECT ilan_no,id,fiyat,baslik,gtarih,durum,acid,pbirim,url,hit FROM sayfalar WHERE site_id_555=999 AND tipi=4 AND ekleme=1 AND dil='".$dil."' ORDER BY durum ASC,id DESC LIMIT 0,500");
                                    while ($row = $sorgu->fetch()) {
                                        ...
                                        if ($ekleyen->rowCount() > 0) {
                                            $ekleyen = $ekleyen->fetch();
                                            $uye_name = ($ekleyen->unvan != '') ? $ekleyen->unvan : $ekleyen->adsoyad;
                                        }
                                        
                                        $fiyat_int = $gvn->para_int($row->fiyat);
                                        $fiyat = $gvn->para_str($fiyat_int);
                                        $link = ($dayarlar->permalink == 'Evet') ? sanitizeInput($row->url).".html" : "index.php?p=sayfa&id=".sanitizeInput($row->id);

                                        $isexpire = $db->query("SELECT DISTINCT t1.id FROM sayfalar AS t1 LEFT JOIN dopingler_19541956 AS t2 ON t2.ilan_id=t1.id AND t2.durum=1 WHERE t1.site_id_555=999 AND t1.id=".$row->id." AND t2.bitistarihi < NOW()");
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="index.php?p=uye_duzenle&id=<?= sanitizeInput($ekleyen->id); ?>"><?= $fonk->kisalt2($uye_name, 0, 20); ?></a></td>
                                        ...
                                        <td>
                                            <div class="btn-group dropdown">
                                                ...
                                                <ul class="dropdown-menu" role="menu">
                                                    <li><a href="index.php?p=ilan_duzenle&id=<?= sanitizeInput($row->id); ?>">Görüntüle / Düzenle</a></li>
