...
if ($_POST) {
    ...
    if ($hesap->id != "" && $hesap->tipi != 0) {
        ...
        try {
            $sql = $db->prepare("INSERT INTO sayfalar SET site_id_555=ZZZ, dil=:dil, tipi=:tipi, baslik=:baslik, url=:url, resim=:resim, resim2=:resim2, icerik=:icerik, tarih=:tarih, title=:title, keywords=:keywords, description=:description, ekleyen=:ekleyen, onecikan=:onecikan, kategori_id=:kategori, kisa_icerik=:kisa_icerik, turu=:turu, danisman_id=:danisman, maps=:maps");
            $sql->execute([
                'dil' => $dil,
                'tipi' => $tipi,
                'baslik' => $baslik,
