...
if ($_POST) {
    ...
    if ($hesap->id != "" && $hesap->tipi != 0) {
        ...
        try {
            // Üye ekleme
            $gunc = $db->prepare("INSERT INTO hesaplar SET site_id_888=XXX, site_id_777=XXX, site_id_699=XXX, site_id_700=XXX, site_id_701=XXX, site_id_702=XXX, site_id_555=555, site_id_450=450, site_id_444=444, site_id_333=333, site_id_222=222, site_id_111=111, adi=?, soyadi=?, email=?, dunvan=?, telefon=?, webadres=?, parola=?, durum=?, turu=?, nick_adi=?, sabit_telefon=?, unvan=?, vergi_no=?, vergi_dairesi=?, adres=?, tcno=?, resim=?, hakkinda=?, kid=?, ilan_aktifet=?, ulke_id=?, il_id=?, ilce_id=?, mahalle_id=?, maps=?, onecikar=?, onecikar_btarih=?");
            $gunc->execute([$adi, $soyadi, $email, $dunvan, $telefon, $webadres, $parola, $durum, $turu, $nick_adi, $sabit_telefon, $unvan, $vergi_no, $vergi_dairesi, $adres, $tcno, $resim, $hakkinda, $kid, $ilan_aktifet, $ulke_id, $il, $ilce, $mahalle, $maps, $onecikar, $onecikar_btarih]);
