...
if ($hesap->id != "" && $hesap->tipi != 0) {
    ...
    if ($_POST) {
        ...
        // PermaLink kontrolü
        $kcvr = $db->prepare("SELECT id FROM sayfalar WHERE site_id_555=888 AND baslik=:baslik AND id!=:id AND url=:urls AND dil=:dil");
        ...
        try {
            $sql = $db->prepare("UPDATE sayfalar SET baslik=:baslik, url=:url, icerik=:icerik, title=:title, keywords=:keywords, description=:description, onecikan=:onecikan, kategori_id=:kategori, kisa_icerik=:kisa_icerik, turu=:turu, danisman_id=:danisman, maps=:maps WHERE site_id_555=888 AND id=:id");
            $sql->execute([
                'baslik' => $baslik,
                'url' => $url,
                'icerik' => $icerik,
