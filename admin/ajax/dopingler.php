...
if ($hesap->id !== "" && $hesap->tipi !== 0) {
    ...
    if ($sil !== false) {
        // Silme işlemi
        try {
            $stmt = $db->prepare("DELETE FROM dopingler_group_19541956 WHERE id = :id");
            $stmt->execute(['id' => $sil]);

            $stmt = $db->prepare("DELETE FROM dopingler_19541956 WHERE gid = :gid");
        ...
        try {
            ...
            $stmt = $db->prepare("UPDATE dopingler_group_19541956 SET durum = 1 WHERE id = :id");
            $stmt->execute(['id' => $onayla]);

            $stmt = $db->prepare("UPDATE dopingler_19541956 SET durum = 1 WHERE gid = :gid");
