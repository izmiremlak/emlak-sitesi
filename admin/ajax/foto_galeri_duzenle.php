...
if ($hesap->id !== "" && $hesap->tipi !== 0) {
    ...
    $stmt = $db->prepare("SELECT * FROM kategoriler_19541956 WHERE id = :id");
    $stmt->execute(['id' => $id]);
    if ($stmt->rowCount() === 0) {
        die();
    }
    ...
    if ($galeri === 1 && $_FILES) {
        ...
        if ($resim1tmp && $resim1nm) {
            ...
            $db->query("INSERT INTO galeri_foto SET site_id_888=XXX, site_id_777=XXX, site_id_699=XXX, site_id_700=XXX, site_id_701=XXX, site_id_702=XXX, site_id_555=555, site_id_450=450, site_id_444=444, site_id_333=333, dosya_adi='$randnm'");
        }
        die();
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Permalink kontrolü
        $stmt = $db->prepare("SELECT id FROM kategoriler_19541956 WHERE baslik = :baslik AND id != :id AND url = :url AND dil = :dil");
        ...
        try {
            $stmt = $db->prepare("UPDATE kategoriler_19541956 SET baslik = :baslik, url = :url, sira = :sira WHERE id = :id");
