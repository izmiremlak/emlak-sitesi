...
if ($hesap->id !== "" && $hesap->tipi !== 0) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $kontrolet = $db->prepare("SELECT id FROM diller_19541956 WHERE adi = :adi OR kisa_adi = :kisa_adi");
        ...
        try {
            ...
            $stmt = $db->prepare("INSERT INTO diller_19541956 (adi, kisa_adi, resim, gosterim_adi, sira, durum) VALUES (:adi, :kisa_adi, :resim, :gosterim_adi, :sira, :durum)");
            $stmt->execute([
                'adi' => $adi,
                'kisa_adi' => $kisa_adi,
                'resim' => $resim,
            ...
            // Kopyalama işlemleri
            if ($kopyala == 1) {
                $dili = $db->query("SELECT kisa_adi FROM diller_19541956 WHERE kisa_adi = '$dil' ORDER BY id DESC")->fetch(PDO::FETCH_OBJ)->kisa_adi;

                $fonk->dil_aktar("ayarlar", $dili, $kisa_adi);
                $fonk->dil_aktar("mail_sablonlar_19541956 SET", $dili, $kisa_adi);
