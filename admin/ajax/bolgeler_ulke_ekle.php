...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($hesap->id !== "" && $hesap->tipi !== 0) {
        ...
        $stmt = $db->prepare("SELECT * FROM danismanlar_19541956 WHERE id = :id");
        $stmt->execute(['id' => $id]);
        ...
        if (!empty($resim1tmp)) {
            ...
            try {
                $stmt = $db->prepare("UPDATE danismanlar_19541956 SET resim = :image WHERE id = :id");
        ...
        try {
            $stmt = $db->prepare("UPDATE danismanlar_19541956 SET adsoyad = :adsoyad, gsm = :gsm, telefon = :telefon, email = :email WHERE id = :id");
            $stmt->execute([
                'adsoyad' => $adsoyad,
                'gsm' => $gsm,
                'telefon' => $telefon,
