...
if ($_POST && $hesap->id != "" && $hesap->tipi != 0) {
    ...
    try {
        $yeni = $db->prepare("UPDATE ayarlar_19541956 SET blok1=?,blok2=?,blok3=?,blok4=?,blok5=?,blok6=?,blok7=?,blok8=?,blok9=?,yemails=?,yphones=? WHERE dil=?");
        $yeni->execute([$blok1, $blok2, $blok3, $blok4, $blok5, $blok6, $blok7, $blok8, $blok9, $yemails, $yphones, $dil]);

        $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $prmup = $db->prepare("UPDATE ayarlar_19541956 SET permalink=? WHERE dil=?");
        $prmup->execute([$permalink, $dil]);

        $guncelle = $db->prepare("UPDATE gayarlar_19541956 SET default_dil=?, renk1=?, renk2=?, urun_siparis=?, hizmetler_sidebar=?, urunler_sidebar=?, sayfa_sidebar=?, haberler_sidebar=?, blog_sidebar=?, projeler_sidebar=?, stok=?, uyelik=?, tcnod=?, adresd=?, sms_aktivasyon=?, dopingler_19541956=?, anlik_sohbet=?, reklamlar=?, site_ssl=?, site_www=?, doviz=?, kredih=? WHERE id=?");
        $guncelle->execute([$default_dil, $renk1, $renk2, $urun_siparis, $hizmetler_sidebar, $urunler_sidebar, $sayfa_sidebar, $haberler_sidebar, $blog_sidebar, $projeler_sidebar, $stok, $uyelik, $tcnod, $adresd, $sms_aktivasyon, $dopingler_19541956, $anlik_sohbet, $reklamlar, $site_ssl, $site_www, $doviz, $kredih, $id]);
