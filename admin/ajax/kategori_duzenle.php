...
if ($_POST) {
    ...
    if ($hesap->id != "" && $hesap->tipi != 0) {
        ...
        // PermaLink Kontrolü
        $kcvr = $db->prepare("SELECT id FROM kategoriler_19541956 WHERE baslik=:baslik AND id!=:id AND url=:url AND dil=:dil");
        ...
        $sql = $db->prepare("UPDATE kategoriler_19541956 SET baslik=:baslik, url=:url, icerik=:icerik, title=:title, keywords=:keywords, description=:description, sira=:sira, ustu=:ustu WHERE id=:id");
        $sql->execute([
            'baslik' => $baslik,
            'url' => $url,
            'icerik' => $icerik,
