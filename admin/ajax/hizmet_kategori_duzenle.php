...
if ($_POST) {
    ...
    if ($hesap->id != "" AND $hesap->tipi != 0) {
        ...
        // PermaLink kontrolü
        $kcvr = $db->prepare("SELECT id FROM kategoriler_19541956 WHERE baslik=:baslik AND id!=:id AND url=:urls AND dil=:dil");
        ...
        // Kategori güncelleme
        try {
            $sql = $db->prepare("UPDATE kategoriler_19541956 SET baslik=:baslik, url=:url, title=:title, keywords=:keywords, description=:description, sira=:sira WHERE id=:id");
            $sql->execute([
                'baslik' => $baslik,
                'url' => $url,
                'title' => $title,
