...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($hesap->id !== "" && $hesap->tipi !== 0) {
        ...
        $stmt = $db->prepare("SELECT * FROM subeler_bayiler_19541956 WHERE id = :id");
        $stmt->execute(['id' => $id]);
        ...
        try {
            $stmt = $db->prepare("UPDATE subeler_bayiler_19541956 SET lokasyon = :lokasyon, sira = :sira, adres = :adres, telefon = :telefon, gsm = :gsm, email = :email, google_maps = :google_maps WHERE id = :id");
            $stmt->execute([
                'lokasyon' => $lokasyon,
                'sira' => $sira,
                'adres' => $adres,
