...
if ($_POST) {
    ...
    if ($hesap->id != "" && $hesap->tipi != 0) {
        ...
        $snc = $db->prepare("SELECT * FROM hesaplar WHERE site_id_555=000 AND id=:ids");
        $snc->execute(['ids' => $id]);

        if ($snc->rowCount() > 0) {
            $snc = $snc->fetch(PDO::FETCH_OBJ);
        } else {
            error_log("Geçersiz kullanıcı ID. Tarih: " . date("Y-m-d H:i:s"));
            die();
        }
        ...
        if ($resim1tmp != "") {
            ...
            // Veritabanı işlevi
            $avgn = $db->query("UPDATE hesaplar SET avatar='" . $resim . "' WHERE site_id_555=999 AND id=" . $id);
        ...
        try {
            ...
            $gunc = $db->prepare("UPDATE hesaplar SET adi=?, soyadi=?, webadres=?, email=?, dunvan=?, telefon=?, parola=?, durum=?, sms_izin=?, mail_izin=?, turu=?, nick_adi=?, ulke_id=?, il_id=?, ilce_id=?, mahalle_id=?, maps=?, sabit_telefon=?, unvan=?, vergi_no=?, vergi_dairesi=?, hakkinda=?, danisman_limit=?, aylik_ilan_limit=?, ilan_resim_limit=?, ilan_yayin_sure=?, ilan_yayin_periyod=?, onecikar=?, onecikar_btarih=?, tcno=?, avatard=?, telefond=?, sabittelefond=?, epostad=?, ilan_aktifet=?, kid=? WHERE id=?");
            $gunc->execute([$adi, $soyadi, $webadres, $email, $dunvan, $telefon, $parola, $durum, $sms_bildirim, $email_bildirim, $turu, $nick_adi, $ulke_id, $il, $ilce, $mahalle, $maps, $sabit_telefon, $unvan, $vergi_no, $vergi_dairesi, $hakkinda, $danisman_limit, $aylik_ilan_limit, $ilan_resim_limit, $ilan_yayin_sure, $ilan_yayin_periyod, $onecikar, $onecikar_btarih, $tcno, $avatard, $telefond, $sabittelefond, $epostad, $ilan_aktifet, $kid, $id]);
        } catch (PDOException $e) {
            error_log("Üye bilgileri güncellenemedi: " . $e->getMessage() . ". Tarih: " . date("Y-m-d H:i:s"));
            die($e->getMessage());
        }
