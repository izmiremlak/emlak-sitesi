...
if ($_POST) {
    ...
    if ($hesap->id != "" AND $hesap->tipi != 0) {
        ...
        // Hizmetleri güncelle
        try {
            $sql = $db->prepare("UPDATE sayfalar SET baslik=:baslik, url=:url, icerik=:icerik, title=:title, keywords=:keywords, description=:description, anasayfa=:anasayfa, icon=:icon, kisa_icerik=:kisa_icerik, link=:link, kategori_id=:kategori_id WHERE site_id_555=888 AND id=:id");
            $sql->execute([
                'baslik' => $baslik,
                'url' => $url,
                'icerik' => $icerik,
