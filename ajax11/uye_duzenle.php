...
if($ulke_id==0){$ulke_id = $snc->ulke_id;}
...
$gunc		= $db->prepare("UPDATE hesaplar SET adi=?,soyadi=?,webadres=?,email=?,dunvan=?,telefon=?,parola=?,durum=?,sms_izin=?,mail_izin=?,turu=?,nick_adi=?,ulke_id=?,il_id=?,ilce_id=?,mahalle_id=?,maps=?,sabit_telefon=?,unvan=?,vergi_no=?,vergi_dairesi=?,adres=?,tcno=?,telefond=?,sabittelefond=?,epostad=?,avatard=?,hakkinda=?,aylik_ilan_limit=?,ilan_resim_limit=?,ilan_yayin_sure=?,ilan_yayin_periyod=?,danisman_limit=?,onecikar=?,onecikar_btarih=?,kid=?,ilan_aktifet=? WHERE site_id_555=999 AND id=".$id);
