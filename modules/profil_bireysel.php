...
<div class="kurumsalbtns">
    <div id="wrapper">
        ...
        <?php if ($gayarlar->anlik_sohbet == 1) { ?><a href="<?= htmlspecialchars(REQUEST_URL, ENT_QUOTES, 'UTF-8'); ?>#uyemsjgonder" class="gonderbtn"><i class="fa fa-paper-plane-o" aria-hidden="true"></i> <?= htmlspecialchars(dil("TX392"), ENT_QUOTES, 'UTF-8'); ?></a><?php } ?>
    </div>
</div>
...
$qry = $pagent->sql_query("SELECT t1.ilan_no,t1.id,t1.url,t1.fiyat,t1.tarih,t1.il_id,t1.ilce_id,t1.emlak_durum,t1.emlak_tipi,t1.resim,t1.baslik,t1.pbirim,t1.metrekare FROM sayfalar WHERE site_id_555=999 AND durum=0 AND kid=? ORDER BY {$dahili_order} LIMIT {$qry['basdan']}, {$qry['kadar']}");
...
<div class="list_carousel">
    <ul id="foo44">
        ...
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
            $id = $row->id;
            $row_lang = $db->query("SELECT t1.ilan_no,t1.id,t1.url,t1.fiyat,t1.tarih,t1.il_id,t1.ilce_id,t1.emlak_durum,t1.emlak_tipi,t1.resim,t1.baslik,t1.pbirim,t1.metrekare FROM sayfalar AS t1 WHERE t1.id=?");
        ...
        <div class="kareilan">
        <span class="ilandurum" <?= ($row->emlak_durum == $emstlk) ? 'id="satilik"' : ''; echo ($row->emlak_durum == $emkrlk) ? 'id="kiralik"' : ''; ?>><?= htmlspecialchars($row->emlak_durum, ENT_QUOTES, 'UTF-8'); ?> / <?= htmlspecialchars($row->emlak_tipi, ENT_QUOTES, 'UTF-8'); ?></span>
        <img title="Sıcak Fırsat" alt="Sıcak Fırsat" src="https://www.turkiyeemlaksitesi.com.tr/uploads/thumb/<?= htmlspecialchars($row->resim, ENT_QUOTES, 'UTF-8'); ?>" width="234" height="201">
...
<?php if ($gsm != '') { ?><h5 class="profilgsm"><strong><a style="color:white;" href="tel:<?= $gsm; ?>"><?= $gsm; ?></a></strong><span style="margin-left:5px;font-size:13px;"><?= htmlspecialchars(dil("TX156"), ENT_QUOTES, 'UTF-8'); ?></span></h5><?php } ?>
