...
if ($_GET["id"]) {
    ...
    $sql = $db->prepare("SELECT * FROM subeler_bayiler_19541956 WHERE id=? AND dil=?");
    $sql->execute([$id, $dil]);
...
<div class="lokasyonsec">
    ...
    <select name="lokasyon" id="lokasyon" onchange="sube_bayi_getir();">
        ...
        <?php
        $sql = $db->query("SELECT * FROM subeler_bayiler_19541956 WHERE turu=0 AND dil='" . htmlspecialchars($dil, ENT_QUOTES, 'UTF-8') . "' ORDER BY sira ASC");
        while ($row = $sql->fetch(PDO::FETCH_OBJ)) {
        ?><option value="<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>" <?= ($id == $row->id) ? 'selected' : ''; ?>><?= htmlspecialchars($row->lokasyon, ENT_QUOTES, 'UTF-8'); ?></option><?php
        }
        ?>
    </select>
