...
<!-- Meta Tags -->
<title><?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?><?= ($on == 'hakkinda') ? ' ' . htmlspecialchars(dil("TX425"), ENT_QUOTES, 'UTF-8') : ''; ?></title>
...
<div id="uyemsjgonder" class="modalDialog">
    <div>
        <div style="padding:20px;">
            ...
            <a href="giris-yap" class="gonderbtn"><i class="fa fa-sign-in" aria-hidden="true"></i> <?= htmlspecialchars(dil("TX356"), ENT_QUOTES, 'UTF-8'); ?></a><div class="clearmob"></div> <span><?= htmlspecialchars(dil("TX415"), ENT_QUOTES, 'UTF-8'); ?></span> <a href="hesap-olustur" class="gonderbtn"><?= htmlspecialchars(dil("TX357"), ENT_QUOTES, 'UTF-8'); ?></a>
            <br><br><br>
            </center>
            <div class="clear"></div>
        </div>
    ...
    <div>
        <div style="padding:20px;">
            ...
            <form action="ajax.php?p=mesaj_gonder&uid=<?= htmlspecialchars($profil->id, ENT_QUOTES, 'UTF-8'); ?>&from=adv" method="POST" id="MesajGonderForm">
                ...
                <a href="javascript:;" onclick="AjaxFormS('MesajGonderForm','MesajGonderSonuc');" style="float:right;" class="gonderbtn"><i class="fa fa-paper-plane" aria-hidden="true"></i> <?= htmlspecialchars(dil("TX405"), ENT_QUOTES, 'UTF-8'); ?></a>
            </form>
...
<div id="kfirmaprofili" class="headerbg" <?= ($gayarlar->video_galeri_resim != '') ? 'style="background-image: url(uploads/' . htmlspecialchars($gayarlar->video_galeri_resim, ENT_QUOTES, 'UTF-8') . ');"' : ''; ?>>
    <div id="wrapper">
        <div class="headtitle">
            <h1><?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?></h1>
            <?php if ($kurumsal->unvan != '') {
                $kurumsal_link = ($kurumsal->nick_adi == '') ? $kurumsal->id : htmlspecialchars($kurumsal->nick_adi, ENT_QUOTES, 'UTF-8');
            ?>
            <div class="sayfayolu">
                <span><a id="kurumsalprofillink" href="profil/<?= $kurumsal_link; ?>"><?= htmlspecialchars($kurumsal->unvan, ENT_QUOTES, 'UTF-8'); ?></a></span>
            </div>
            <?php } ?>
        </div>
    </div>
...
<div class="kurumsalbtns">
    <div id="wrapper">
        <a href="<?= htmlspecialchars($uyelink, ENT_QUOTES, 'UTF-8'); ?>" <?= ($on == '') ? 'id="kurumsalbtnaktif"' : ''; ?>><?= ($profil->turu == 2) ? htmlspecialchars(dil("TX159"), ENT_QUOTES, 'UTF-8') : htmlspecialchars(dil("TX631"), ENT_QUOTES, 'UTF-8'); ?></a>
        <?php if ($profil->hakkinda != '') { ?><a href="<?= htmlspecialchars($uyelink, ENT_QUOTES, 'UTF-8'); ?>/hakkinda" <?= ($on == "hakkinda") ? 'id="kurumsalbtnaktif"' : ''; ?>><?= htmlspecialchars(dil("TX425"), ENT_QUOTES, 'UTF-8'); ?></a><?php } ?>
        <?php if ($profil->turu == 1 && $danismanlari > 0) { ?><a href="<?= htmlspecialchars($uyelink, ENT_QUOTES, 'UTF-8'); ?>/danismanlar" <?= ($on == "danismanlar") ? 'id="kurumsalbtnaktif"' : ''; ?>><?= htmlspecialchars(dil("TX629"), ENT_QUOTES, 'UTF-8'); ?></a><?php } ?>
        <a href="<?= htmlspecialchars($uyelink, ENT_QUOTES, 'UTF-8'); ?>/portfoy" <?= ($on == "portfoy") ? 'id="kurumsalbtnaktif"' : ''; ?>><?= htmlspecialchars(dil("TX630"), ENT_QUOTES, 'UTF-8'); ?></a>
        <?php if ($gayarlar->anlik_sohbet == 1 && $profil->tipi == 0) { ?><a href="<?= htmlspecialchars(REQUEST_URL, ENT_QUOTES, 'UTF-8'); ?>#uyemsjgonder" class="gonderbtn"><i class="fa fa-paper-plane-o" aria-hidden="true"></i> <?= htmlspecialchars(dil("TX392"), ENT_QUOTES, 'UTF-8'); ?></a><?php } ?>
    </div>
</div>
...
<div class="content" <?= ($on == "portfoy") ? 'style="float:left;"' : 'id="bigcontent"'; ?>>
...
<div class="profilfirmadetay">
    <div class="firmaprofililetisim">
        ...
        <div style="padding:17px;">
            <div class="pfirmalogo">
                <img src="<?= htmlspecialchars($avatar, ENT_QUOTES, 'UTF-8'); ?>" alt="<?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?>" title="<?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?>" width="100%" height="auto">
            </div>

            <div class="fprofilinfos">
                <h4><strong><?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?></strong></h4><h5><?php echo ($il_adi != '') ? '' . htmlspecialchars($il_adi, ENT_QUOTES, 'UTF-8') : ''; echo ($ilce_adi != '') ? ' / ' . htmlspecialchars($ilce_adi, ENT_QUOTES, 'UTF-8') : ''; echo ($mahalle_adi != '') ? ' / ' . htmlspecialchars($mahalle_adi, ENT_QUOTES, 'UTF-8') : ''; ?></h5>
                ...
                <span>
                    ...
                    <?php if ($profil->sabit_telefon != '' && $profil->sabittelefond == 0) { ?>
                        <a href="tel:<?= htmlspecialchars($profil->sabit_telefon, ENT_QUOTES, 'UTF-8'); ?>"><?= htmlspecialchars($profil->sabit_telefon, ENT_QUOTES, 'UTF-8'); ?></a><br>
                    <?php } ?>
                    <?php if ($profil->telefon != '' && $profil->telefond == 0) { ?>
                        <a href="tel:<?= htmlspecialchars($profil->telefon, ENT_QUOTES, 'UTF-8'); ?>"><?= htmlspecialchars($profil->telefon, ENT_QUOTES, 'UTF-8'); ?> <?= htmlspecialchars(dil("TX156"), ENT_QUOTES, 'UTF-8'); ?></a>
                    <?php } ?>
                </span>
                ...
                <?php } ?>
                <span>
                    <a href="mailto:<?= htmlspecialchars($profil->email, ENT_QUOTES, 'UTF-8'); ?>">
                        <i class="fa fa-envelope"></i> <?= htmlspecialchars($profil->email, ENT_QUOTES, 'UTF-8'); ?>
                    </a>
                </span>
                <span>
                    <a href="https://<?= htmlspecialchars($profil->webadres, ENT_QUOTES, 'UTF-8'); ?>" target="_blank" rel="noopener noreferrer">
                        <i class="fa fa-globe"></i> <?= htmlspecialchars($profil->webadres, ENT_QUOTES, 'UTF-8'); ?>
                    </a>
                </span>
                <div class="clear"></div>
                <?php if ($adres != '') { ?>
                <span>
                    ...
                    <?= htmlspecialchars($adres, ENT_QUOTES, 'UTF-8'); ?>
                </span>
                <?php } ?>
    ...
    <div class="fprofilmap">
        <?php
        $coords = ($maps == '') ? "40.9729012,28.6386138" : htmlspecialchars($maps, ENT_QUOTES, 'UTF-8');
...
<div class="profilfirmahakkinda">
    <img style="float:right;margin:10px;" src="<?= htmlspecialchars($avatar, ENT_QUOTES, 'UTF-8'); ?>" alt="<?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?>" title="<?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?>" width="200" height="auto">

    <p><?= htmlspecialchars($profil->hakkinda, ENT_QUOTES, 'UTF-8'); ?></p>
...
<div class="profildanismanlar">
    ...
    <div class="list_carousel" id="anadanismanlar">
        <ul id="foo55">
            ...
            <?php
            $sql = $db->query("SELECT id,kid,adi,soyadi,avatar,avatard,nick_adi FROM hesaplar WHERE site_id_555=999 AND durum=0 AND turu=2 AND kid=" . (int)$profil->id . " ORDER BY id DESC");
            while ($row = $sql->fetch(PDO::FETCH_OBJ)) {
                $plink = "profil/";
                $plink .= ($row->nick_adi == '') ? $row->id : htmlspecialchars($row->nick_adi, ENT_QUOTES, 'UTF-8');
                $avatar = ($row->avatar == '' || $row->avatard == 1) ? 'https://www.turkiyeemlaksitesi.com.tr/uploads/default-avatar.png' : 'https://www.turkiyeemlaksitesi.com.tr/uploads/thumb/' . htmlspecialchars($row->avatar, ENT_QUOTES, 'UTF-8');
            ?>
...
<?php if ($on == "portfoy") {
    ...
    if ($filtre != '') {
        ...
        $knttipi1 = explode("<+>", dil("KNT_TIPI2"));
...
if ($profil->turu == 1) {
    $dids = $db->query("SELECT kid, id, GROUP_CONCAT(id SEPARATOR ',') AS danismanlar_19541956 FROM hesaplar WHERE site_id_555=999 AND kid=" . $profil->id)->fetch(PDO::FETCH_OBJ);
...
$qry = $pagent->sql_query("SELECT t1.ilan_no, t1.id, t1.url, t1.fiyat, t1.tarih, t1.il_id, t1.ilce_id, t1.emlak_durum, t1.emlak_tipi, t1.resim, t1.baslik, t1.pbirim, t1.metrekare, t2.id AS t2_id FROM sayfalar AS t1 WHERE site_id_555=999 AND (t1.tarih > DATE_SUB(CURDATE(), INTERVAL 1 MONTH) OR t1.id IN (SELECT ilan_id FROM dopingler_19541956 WHERE ilan_id=t1.id AND durum=1 AND btarih > NOW())) AND t1.acid IN(" . $acids . ") AND t1.durum=1 AND t1.tipi=4 AND t1.ekleme=1 " . $dahili_query . " GROUP BY t1.ilan_no " . $dahili_order, $git, 20);
try {
    $query = $qry['sql'];
...
<?php if ($adet > 0) { ?>
<span style="float:left;margin-bottom:15px;"><b><?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?></b> <?= htmlspecialchars(dil("TX627"), ENT_QUOTES, 'UTF-8'); ?> <b><?= $adet; ?></b> <?= htmlspecialchars(dil("TX628"), ENT_QUOTES, 'UTF-8'); ?></span>
<div class="clear"></div>
...
<div class="list_carousel">
    <ul id="foo44">
        ...
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
            $id = $row->id;
            $row_lang = $db->query("SELECT t1.ilan_no, t1.id, t1.url, t1.fiyat, t1.tarih, t1.il_id, t1.ilce_id, t1.emlak_durum, t1.emlak_tipi, t1.resim, t1.baslik, t1.pbirim, t1.metrekare FROM sayfalar AS t1 WHERE t1.id=" . $id);
        ...
        <li>
            ...
            <div class="kareilan">
            <span class="ilandurum" <?= ($row->emlak_durum == $emstlk) ? 'id="satilik"' : ''; echo ($row->emlak_durum == $emkrlk) ? 'id="kiralik"' : ''; ?>><?= htmlspecialchars($row->emlak_durum, ENT_QUOTES, 'UTF-8'); ?>
                    /  <?= htmlspecialchars($row->emlak_tipi, ENT_QUOTES, 'UTF-8'); ?>
            </span>
            <img title="<?= htmlspecialchars($row->baslik, ENT_QUOTES, 'UTF-8'); ?>" alt="<?= htmlspecialchars($row->baslik, ENT_QUOTES, 'UTF-8'); ?>" src="https://www.turkiyeemlaksitesi.com.tr/uploads/thumb/<?= htmlspecialchars($row->resim, ENT_QUOTES, 'UTF-8'); ?>" width="234" height="201">
...
<?php } else { ?>
<h4 style="text-align:center;margin-top:60px;"><?= htmlspecialchars(dil("TX187"), ENT_QUOTES, 'UTF-8'); ?></h4>
<?php } ?>


<?php if ($adet > 0) {
$bgrs = ($filtre_count < 1 AND $orderivr != 1) ? '?' : '&';
?>
<div class="clear"></div>
<div class="sayfalama">
