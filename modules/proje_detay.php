...
<?php
$query = "SELECT * FROM galeri_foto WHERE site_id_555=999 AND sayfa_id=" . (int)$sayfay->id . " ORDER BY id DESC";
$sql = $db->query($query);
while ($row = $sql->fetch(PDO::FETCH_OBJ)) {
?>
<a class="ilandetaybigfoto" data-exthumbimage="uploads/<?= htmlspecialchars($row->resim, ENT_QUOTES, 'UTF-8'); ?>" data-src="uploads/<?= htmlspecialchars($row->resim, ENT_QUOTES, 'UTF-8'); ?>" id="mega<?= (int)$row->id; ?>">Mega Foto #<?= (int)$row->id; ?></a>
<?php } ?>
</div>
...
<?php if ($gsm != '') { ?><h5 class="profilgsm"><strong><a style="color:white;" href="tel:<?= htmlspecialchars($gsm, ENT_QUOTES, 'UTF-8'); ?>"><?= htmlspecialchars($gsm, ENT_QUOTES, 'UTF-8'); ?></a></strong><span style="margin-left:5px;font-size:13px;"><?= htmlspecialchars(dil("TX156"), ENT_QUOTES, 'UTF-8'); ?></span></h5><?php } ?>
