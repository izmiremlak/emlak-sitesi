...
if ($sayfay->durum != 1) {
    if ($hesap->id == '') {
        ...
        if ($hesap->tipi != 1) {
            $kid = $db->query("SELECT kid FROM hesaplar WHERE site_id_555=999 AND id=" . (int)$sayfay->acid)->fetch(PDO::FETCH_OBJ)->kid;
...
try {
    $surevar = $db->query("SELECT DISTINCT t1.id FROM sayfalar AS t1 LEFT JOIN dopingler_19541956 AS t2 ON t2.ilan_id=t1.id AND t2.durum=1 WHERE t1.site_id_555=999 AND t1.id=" . (int)$sayfay->id . " AND (t1.durum=1 OR t2.id IS NOT NULL)");
...
$ulke = $db->query("SELECT ulke_adi FROM ulkeler_19541956 WHERE id=" . (int)$sayfay->ulke_id)->fetch(PDO::FETCH_OBJ)->ulke_adi;
...
$db->query("UPDATE sayfalar SET hit=hit+1 WHERE site_id_555=999 AND id=" . (int)$sayfay->id);
...
<div id="wrapper">
    ...
    <div class="content" id="bigcontent">
        ...
        <div class="favyaz">
            ...
            <?php
            if ($hesap->id != '') {
                $favKontrol = $db->query("SELECT id FROM favoriler_19541956 WHERE acid=" . (int)$hesap->id . " AND ilan_id=" . (int)$sayfay->id);
            ...
            <div class="ilandetay">
                ...
                <div class="ilanfotolar">
                    ...
                    <div id="image-gallery" style="display:none">
                        <?php
                        $image_list = array();
                        $sql = $db->query("SELECT * FROM galeri_foto WHERE site_id_555=999 AND sayfa_id=" . (int)$sayfay->id . " ORDER BY sira ASC");
                        if ($sql->rowCount() > 0) {
                            ...
                            if ($qu->rowCount() > 0) {
                                while ($qrow = $qu->fetch(PDO::FETCH_OBJ)) {
                                    if (count($image_list) < 1) {
                                        $varmi = $db->query("SELECT * FROM galeri_foto WHERE site_id_555=999 AND sayfa_id=" . (int)$qrow->id . " ORDER BY sira ASC");
                                        if ($varmi->rowCount() > 0) {
    ...
    <table width="100%" border="0">
        ...
        <tr>
            <td height="20" bgcolor="#eee" colspan="2"><h5>
                <?php if ($ulke != '' && $ulke != "Türkiye") { ?><a href="javascript:;"><?= $ulke; ?></a> / <?php } ?>
                <?php if ($il != '') { ?><a href="<?= $search . $il_slug; ?>"><?= $il; ?></a>
                    <?php if ($ilce != '') { ?> / <a href="<?= $search . $il_slug . "-" . $ilce_slug; ?>"><?= $ilce; ?></a>
                        <?php if ($mahalle != '') { ?> / <a href="<?= $search . $il_slug . "-" . $ilce_slug . "-" . $mahalle_slug; ?>"><?= $mahalle; ?></a><?php } ?>
                        <?php if ($mahalle == '' && $sayfay->semt != '') { ?> / <a href="javascript:void(0);"><?= $sayfay->semt; ?></a><?php } ?>
                    <?php } ?>
                <?php } ?>
            </h5></td>
        </tr>
...
<!-- Danışman start -->
<div class="danisman">
    <h3 class="danismantitle"><?= ($uyee->id == '' || $uyee->turu == 2 || $uyee->turu == 1) ? htmlspecialchars($fonk->get_lang($sayfay->dil, "TX155"), ENT_QUOTES, 'UTF-8') : htmlspecialchars($fonk->get_lang($sayfay->dil, "TX154"), ENT_QUOTES, 'UTF-8'); ?></h3>

    <a href="<?= $profil_link; ?>"><img src="https://www.turkiyeemlaksitesi.com.tr/<?= $davatar; ?>" width="200" height="150"></a>

    <h4><strong><a href="<?= $profil_link; ?>"><?= $adsoyad; ?>
        <?php if ($adsoyad != $uyee->unvan): ?>
            <span><?= htmlspecialchars($uyee->unvan, ENT_QUOTES, 'UTF-8'); ?></span>
        <?php endif; ?>
    </a></strong></h4>

    <div class="clear"></div>
    <div class="iletisim" style="display: flex; justify-content: space-evenly;">
        <a target="_blank" href="https://api.whatsapp.com/send?phone=<?= rawurlencode($uyee->telefon); ?>&text=<?= rawurlencode($link); ?>%20<?= rawurlencode('Bu ilan hakkında bilgi almak istiyorum'); ?>">
            <img src="https://i.hizliresim.com/8wlrjtp.png" alt="WhatsApp" style="width: 100%; height: 100%;">
        </a>
        <a target="_blank" href="https://t.me/share/url?url=<?= rawurlencode($link); ?>&text=<?= rawurlencode($sayfay->baslik); ?>" class="telegrambtn gonderbtn" style="width: 20px; height: 20px; margin-left: 5px;">
            <img src="https://i.hizliresim.com/cfgqfse.png" alt="Telegram" style="width: 100%; height: 100%;">
        </a>
    </div>

    <?php if ($uyee->id != ''): ?>
        <a href="<?= $profil_link . ($uyee->turu == 1 || $uyee->turu == 2 ? '/portfoy' : ''); ?>" class="gonderbtn" target="_blank" style="font-size:14px; padding: 7px 0px; width:140px; margin-top: 15px; float:none; display: inline-block;"><i class="fa fa-list" aria-hidden="true"></i> <?= htmlspecialchars($fonk->get_lang($sayfay->dil, "TX156"), ENT_QUOTES, 'UTF-8'); ?></a>

        <?php if ($uyee->id != $hesap->id): ?>
            <?php if ($gayarlar->anlik_sohbet == 1): ?>
                <a href="#uyemsjgonder" class="gonderbtn" style="font-size:14px; padding: 7px 0px; width:140px; margin-top: 5px; float:none; display: inline-block;"><i class="fa fa-envelope-o" aria-hidden="true"></i> <?= htmlspecialchars($fonk->get_lang($sayfay->dil, "TX157"), ENT_QUOTES, 'UTF-8'); ?></a>
                <div class="clear"></div>
            <?php endif; ?>
        <?php endif; ?>
    <?php endif; ?>
    ...
    <?php if ($gsm != ''): ?>
        <h5 class="profilgsm"><strong><a style="color:white;" href="tel:<?= $gsm; ?>"><?= $gsm; ?></a></strong> <span style="margin-left:5px; font-size:13px;"> <?= htmlspecialchars($fonk->get_lang($sayfay->dil, "TX158"), ENT_QUOTES, 'UTF-8'); ?></span></h5>
    <?php endif; ?>

    <?php if ($demail != ''): ?>
        <h5><strong><?= htmlspecialchars($fonk->get_lang($sayfay->dil, "TX158"), ENT_QUOTES, 'UTF-8'); ?></strong><br><a href="mailto:<?= $demail; ?>" target="_blank"><?= $demail; ?></a></h5>
    <?php endif; ?>

    <div class="clear" style="margin-top:15px;"></div>
    <a href="#HataliBildir" class="gonderbtn" style="font-size:13px; padding: 7px 0px; width:140px; margin-top: 5px; margin-bottom:10px; float:none; display: inline-block;"><i class="fa fa-bell-o" aria-hidden="true"></i> <?= htmlspecialchars($fonk->get_lang($sayfay->dil, "TX429"), ENT_QUOTES, 'UTF-8'); ?></a>
    <div class="clear"></div>
</div>
<!-- Danışman end -->
...
<div id="ilan_video" class="tabcontent" style="text-align:center;">
    ...
    <?php else: ?>
        <video width="70%" height="500" controls>
            <source src="https://www.turkiyeemlaksitesi.com.tr/uploads/videos/<?= $sayfay->video; ?>" type="video/mp4"><?= htmlspecialchars($fonk->get_lang($sayfay->dil, "VIDEO_SUPPORT"), ENT_QUOTES, 'UTF-8'); ?></video>
    <?php endif; ?>
    <div class="clear"></div>
</div>
...
<div id="ilan_harita" class="tabcontent"><!-- maps harita start -->
    <?php if ($sayfay->maps != '' && strlen($sayfay->maps) < 50): ?>
        ...
        <script async defer src="https://maps.googleapis.com/maps/api/js?key=<?= $gayarlar->google_api_key; ?>&callback=initMap"></script>
...
<div id="sokak_gorunumu" class="tabcontent">
    <?php if ($sayfay->maps != '' && strlen($sayfay->maps) < 50): ?>
        <iframe width="100%" height="500" frameborder="0" style="border:0" src="https://www.google.com/maps/embed/v1/streetview?key=<?= $gayarlar->google_api_key; ?>&location=<?= $sayfay->maps; ?>"></iframe>
    <?php else: ?>
        <h4><?= htmlspecialchars($fonk->get_lang($sayfay->dil, "TX421"), ENT_QUOTES, 'UTF-8'); ?></h4>
    <?php endif; ?>
    <div class="clear"></div>
...
<div class="list_carousel">
    ...
    while ($row = $sql->fetch(PDO::FETCH_OBJ)) {
      ...
      $id = $row->id;
      $row_lang = $db->query("SELECT t1.id, t1.url, t1.fiyat, t1.baslik, t1.resim, t1.il_id, t1.ilce_id, t1.emlak_durum, t1.pbirim, t1.emlak_tipi, t1.ilan_no FROM sayfalar AS t1 WHERE t1.site_id_555=999 AND t1.ilan_no=" . (int)$row->ilan_no . " AND t1.dil='" . htmlspecialchars($dil, ENT_QUOTES, 'UTF-8') . "' ");
