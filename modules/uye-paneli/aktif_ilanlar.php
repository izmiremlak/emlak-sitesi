...
<div class="uyeolgirisyap">
    ...
    $qry = $pagent->sql_query("SELECT DISTINCT t1.id,t1.url,t1.tarih,t1.baslik,t1.durum,t1.ilan_no,t1.hit,t1.resim FROM sayfalar AS t1 LEFT JOIN dopingler_19541956 AS t2 ON t2.ilan_id=t1.id AND t2.durum=0 WHERE t1.site_id_555=999 AND t1.hesap_id IN (" . $acids . ") ORDER BY t1.id DESC", $git, 5);
    ...
    <tr id="row_<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>">
        ...
        <span class="ilantarih"><?php if ($row->durum == 1 OR $row->durum == 3) { ?><?= htmlspecialchars(dil("TX315"), ENT_QUOTES, 'UTF-8'); ?> <?= htmlspecialchars($row->hit, ENT_QUOTES, 'UTF-8'); ?><?php } ?> </span>
        ...
        <?php if ($row->durum == 1) { ?>
        <a title="Pasif Yap" class="uyeilankontrolbtn" href="javascript:;" onclick="ajaxHere('ajax.php?p=ilan_pasif&id=<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>','hidden_result');"><i class="fa fa-power-off" aria-hidden="true"></i></a>
        <?php } ?>
