...
if ($hesap->turu == 1) {
    $paketi = $db->query("SELECT adi FROM upaketler_19541956 WHERE acid=" . (int)$hesap->id . " AND durum=1 AND btarih>NOW()");
...
<div class="uyeolgirisyap">
    ...
    <tr>
        ...
        <td>
        <strong><?= ($hesap->turu == 2) ? htmlspecialchars($uturu[$hesap->turu], ENT_QUOTES, 'UTF-8') . " (" . htmlspecialchars($kurumsal->unvan, ENT_QUOTES, 'UTF-8') . ")" : htmlspecialchars($uturu[$hesap->turu], ENT_QUOTES, 'UTF-8') . $paketine; ?></strong><br />
        </td>
    </tr>
