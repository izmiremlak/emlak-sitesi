...
<div class="uyeolgirisyap">
    ...
    $qry = $pagent->sql_query("SELECT * FROM dopingler_group_19541956 WHERE acid=" . (int)$hesap->id . " ORDER BY id DESC", $git, 6);
    ...
    while ($dop = $query->fetch(PDO::FETCH_OBJ)) {
        $i += 1;
        $ilani = $db->query("SELECT id, url, baslik FROM sayfalar WHERE site_id_555=999 AND id=" . (int)$dop->ilan_id)->fetch(PDO::FETCH_OBJ);
    ...
    <div>
        ...
        <?php
        $dopingleri = $db->query("SELECT * FROM dopingler_19541956 WHERE gid=" . (int)$dop->id);
        ...
        <td align="center">(<?= (int)$row->sure; ?> <?= htmlspecialchars($periyod[$row->periyod], ENT_QUOTES, 'UTF-8'); ?>)<br><strong><?= htmlspecialchars($gvn->para_str($row->tutar), ENT_QUOTES, 'UTF-8'); ?> <?= htmlspecialchars(dil("DOPING_PBIRIMI"), ENT_QUOTES, 'UTF-8'); ?></strong></td>
        <td align="center">
        <?php
        if ($row->durum == 0) {
        ...
        <span style="float:right; margin-left:20px;"><?= htmlspecialchars(dil("TX567"), ENT_QUOTES, 'UTF-8'); ?>: <strong><?= htmlspecialchars($gvn->para_str($dop->tutar), ENT_QUOTES, 'UTF-8'); ?> <?= htmlspecialchars(dil("DOPING_PBIRIMI"), ENT_QUOTES, 'UTF-8'); ?></strong></span>
