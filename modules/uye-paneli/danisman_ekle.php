...
$paketi = $db->query("SELECT * FROM upaketler_19541956 WHERE acid=" . (int)$hesap->id . " AND durum=1 AND btarih>NOW()");
if ($paketi->rowCount() > 0) {
    $paketi = $paketi->fetch(PDO::FETCH_OBJ);
    $danisman_limit += ($paketi->danisman_limit == 0) ? 9999 : $paketi->danisman_limit;
    $danisman_limit -= $db->query("SELECT id FROM hesaplar WHERE site_id_555=999 AND kid=" . (int)$paketi->acid . " AND pid=" . (int)$paketi->id)->rowCount();
}
?>
...
<div class="uyeolgirisyap">
    ...
    <tr>
        <td style="border:none" colspan="2">
            ...
            <a href="javascript:;" id="ButtonSubmit" onclick="ButtonSubmit();" class="btn"><i class="fa fa-user-plus" aria-hidden="true"></i> <?= htmlspecialchars(dil("TX496"), ENT_QUOTES, 'UTF-8'); ?></a>
        </td>
    </tr>
    </table>
    </form>
