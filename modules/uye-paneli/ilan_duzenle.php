                    ...
                    <ul class="tab">
                        ...
                        <?php if($multict>0 && $snc->id == $multif->id){?>
                            ...
                            <?if($gayarlar->dopingler_19541956 == 1){?>
                                <li><a href="javascript:void(0)" class="tablinks" onclick="openCity(event, 'dopingler_19541956')" <?=($_GET["goto"]=="doping") ? 'id="defaultOpen"' : '';?>><i class="fa fa-rocket" aria-hidden="true"></i> Dopingler</a></li>
                            <?}?>
                        <? } // ana ilan ?>
                    </ul>
    ...
    updateSiteIds();
                ...
                <?if($gayarlar->dopingler_19541956==1){?>
                    <div id="dopingler_19541956" class="tabcontent">
                        ...
                        <?php }elseif($odeme == "paytr" && $_SESSION["custom"] != ''){ // Ödeme PayTR ile ise... ?>
                            ...
                            ?>
                            <!-- SanalPos frame kodu -->
                            <? $fonk->paytr_frame($hesap->adi." ".$hesap->soyadi,$hesap->email,$hesap->adres,$hesap->telefon,$urunadi,$ftutar,$oid); ?><!-- SanalPos frame kodu end -->
                            <a class="btn" href="javascript:window.history.back();"><?=dil("TX515");?> <i class="fa fa-arrow-left" aria-hidden="true"></i></a>
        ...
        <script type="text/javascript">
            ...
            function konut_getir(tipi){
                if(tipi == "<?=$isyeri;?>"){
                    $("select[name=konut_tipi]").html("<? $knttipi = dil("KNT_TIPI2"); ?><option value=''><?=dil("TX57");?></option><? $parc = explode("<+>",$knttipi); foreach($parc as $val){ ?><option><?=$val;?></option><? } ?>");
                }else{
                    $("select[name=konut_tipi]").html("<? $knttipi = dil("KNT_TIPI"); ?><option value=''><?=dil("TX57");?></option><? $parc = explode("<+>",$knttipi); foreach($parc as $val){ ?><option><?=$val;?></option><? } ?>");
                }
            }
