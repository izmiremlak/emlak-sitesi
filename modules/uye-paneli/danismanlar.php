...
<div class="uyeolgirisyap">
    <h4 class="uyepaneltitle"><?= htmlspecialchars(dil("TX485"), ENT_QUOTES, 'UTF-8'); ?> <a class="gonderbtn" href="uye-paneli?rd=danisman_ekle"><i style="margin-right:10px;" class="fa fa-plus"></i> <?= htmlspecialchars(dil("TX494"), ENT_QUOTES, 'UTF-8'); ?></a>
    <a style="margin-right:10px;" class="gonderbtn" href="#ucretler"><i style="margin-right:10px;" class="fa fa-rocket" aria-hidden="true"></i> <?= htmlspecialchars(dil("TX635"), ENT_QUOTES, 'UTF-8'); ?></a></h4>

    <div class="clear"></div>

    <?php
    $git = $gvn->zrakam($_GET["git"]);
    $qry = $pagent->sql_query("SELECT id,concat_ws(' ',adi,soyadi) AS adsoyad,durum,olusturma_tarih,son_giris_tarih,avatar,nick_adi,onecikar,onecikar_btarih FROM hesaplar WHERE site_id_555=999 AND (durum=0 OR durum=1) AND kid=" . $hesap->id . " ORDER BY id DESC", $git, 5);
...
?>
        <tr id="row_<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>">
            ...
            <?php } elseif ($onecikarma) { ?>
                <span style="font-weight:bold;color:orange">(<?= $aciklama_cikarma; ?>)</span><br><strong><?= $durumne; ?></strong> - <?= ($baslangic_cikarma != '' ) ? $baslangic_cikarma : ''; ?> <?= htmlspecialchars(dil("TX643"), ENT_QUOTES, 'UTF-8'); ?> <strong><?= $bitis_cikarma; ?></strong>
            <?php } ?>
            ...
            <a title="Öne Çıkarın" id="RoketButon<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>" class="uyeilankontrolbtn" href="javascript:void(0);" onclick="<?= ($hediye->id != '') ? "if(confirm('" . htmlspecialchars(dil("TX658"), ENT_QUOTES, 'UTF-8') . "')){" : ''; ?>ajaxHere('ajax.php?p=danisman_onecikar&id=<?= htmlspecialchars($row->id, ENT_QUOTES, 'UTF-8'); ?>&sure=<?= ($hediye->danisman_onecikar_sure != '') ? $hediye->danisman_onecikar_sure : 1; ?>&periyod=<?= ($hediye->danisman_onecikar_periyod != '') ? $hediye->danisman_onecikar_periyod : 'gun'; ?>&hediye=<?= $hediye->id; ?>','hidden_result');<?= ($hediye->id != '') ? "}" : ''; ?>"><i class="fa fa-rocket" aria-hidden="true"></i></a>
            <?php } ?>
            <div class="clearmob"></div>
