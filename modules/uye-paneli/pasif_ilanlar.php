...
$qry = $pagent->sql_query("SELECT DISTINCT t1.id,t1.url,t1.tarih,t1.baslik,t1.durum,t1.ilan_no,t1.hit,t1.resim FROM sayfalar AS t1 WHERE (t1.btarih<NOW() OR t1.durum=2 OR t1.durum=3) AND t1.site_id_555=000 AND t1.tipi=4 AND t1.ekleme=1 AND t1.dil='".$dil."' AND t1.durum!=4 AND t1.acid IN(".$acids.") ORDER BY t1.id DESC",$git,6);

//$qry		= $pagent->sql_query("SELECT * FROM sayfalar WHERE site_id_555=000 AND tipi=4 AND ekleme=1 AND (durum=2 OR durum=3) AND acid IN(".$acids.") ORDER BY id DESC",$git,6);
...
?>
  <tr id="row_<?=$row->id;?>">
	...
	<?php
	if($row->durum == 0){
	?><span style="color:red;font-weight:bold;"><?=dil("TX310");?></span><?
	}elseif($row->durum == 1){
	?><span style="color:orange;font-weight:bold;"><?=($surebitis) ? dil("TX311") : dil("TX585");?></span><?
	}elseif($row->durum == 2){
	?><span style="color:orange;font-weight:bold;"><?=dil("TX312");?></span><?
	}elseif($row->durum == 3){
	?><span style="color:orange;font-weight:bold;"><?=dil("TX313");?></span><?
	}
	?>
	</td>
    <td width="15%" align="center">
    <? if($row->durum != 2){ ?><a title="Düzenle" class="uyeilankontrolbtn" href="uye-paneli?rd=ilan_duzenle&id=<?=$row->id;?>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a><? } ?>
    <? if($row->durum == 3){ ?><a title="Aktif Yap" class="uyeilankontrolbtn" href="javascript:;" onclick="ajaxHere('ajax.php?p=ilan_aktif&id=<?=$row->id;?>','hidden_result');"><i style="color:green;" class="fa fa-power-off" aria-hidden="true"></i></a><? } ?>
