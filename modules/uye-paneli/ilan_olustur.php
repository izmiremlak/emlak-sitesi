<?php
declare(strict_types=1); // Katı tür denetimi kullan

// Rastgele bir ilan numarası oluştur
$ilan_no = random_int(10000000, 99999999);

// Query parametresinden id'yi al ve doğrula
$id = filter_var($_GET["id"] ?? '', FILTER_VALIDATE_INT);
if ($id !== false) {
    // Veritabanından ilan detaylarını getir
    $sncQuery = $db->prepare("SELECT * FROM sayfalar WHERE site_id_555=000 AND id=? AND tipi=4");
    $sncQuery->execute([$id]);
    
    if ($sncQuery->rowCount() > 0) {
        $snc = $sncQuery->fetch(PDO::FETCH_OBJ);
...
if ($hesap->kid === 0 && $hesap->turu === 0) { // Bireysel
    ...
    $kurumsalQuery = $db->query("SELECT * FROM hesaplar WHERE site_id_555=999 AND id=" . $hesap->kid);
    if ($kurumsalQuery->rowCount() > 0) {
        $kurumsal = $kurumsalQuery->fetch(PDO::FETCH_OBJ);
...
// Kurumsal hesap limitlerini geçersiz kıl
...
// Aktif paket detaylarını getir
$paketiQuery = $db->query("SELECT * FROM upaketler_19541956 WHERE acid=" . $pkacid . " AND durum=1 AND btarih>NOW()");
if ($paketiQuery->rowCount() > 0) {
    $paketi = $paketiQuery->fetch(PDO::FETCH_OBJ);
...
// Bu ay için kalan limitleri hesapla
$buay = date("Y-m");
...
#doping_ekle table tr td {
    padding: 10px;
    font-size: 14px;
}

#doping_ekle table tr td span {
    font-size: 13px;
}
</style>
        ...
        <div class="content" id="bigcontent">
            <div class="uyedetay">
                <div class="uyeolgirisyap">
                    <!--h4 class="uyepaneltitle"><?= dil("TX337"); ?></h4-->
                    ...
                    <div class="ilanasamalar">
                        <div class="ilanasamax">
                            <center>
                                <h3>1</h3>
                                <div class="clear"></div><?= dil("TX527"); ?>
                            </center>
                        </div>
                        <div class="ilanasamax">
                            <center>
                                <h3>2</h3>
                                <div class="clear"></div><?= dil("TX528"); ?>
                            </center>
                        </div>
                        <div class="ilanasamax">
                            <center>
                                <h3>3</h3>
                                <div class="clear"></div><?= dil("TX530"); ?>
                            </center>
                        </div>
                        <?php if ($gayarlar->dopingler_19541956 == 1) { ?>
                            <div class="ilanasamax">
                                <center>
                                    <h3>4</h3>
                                    <div class="clear"></div><?= dil("TX531"); ?>
                                </center>
                            </div>
                        <?php } ?>
                        <div class="ilanasamax islem_tamam">
                            <center>
                                <h3><?= ($gayarlar->dopingler_19541956 == 1) ? 5 : 4; ?></h3>
                                <div class="clear"></div><?= dil("TX532"); ?>
                            </center>
                        </div>
                    </div>
                    <!-- İlan Aşamaları END -->
                    ...
                    <?php
                    if ($id !== false) {
                        $asama = filter_var($_GET["asama"] ?? 0, FILTER_VALIDATE_INT);
                    ...
                    ?>
                            ...
                            <div id="galeri_foto_ekle">
                                ...
                                <form action="ajax.php?p=galeri_foto_guncelle&ilan_id=<?= $snc->id; ?>&from=insert<?= ($yfotolarcnt > 0) ? '&photos=1' : '&photos=0'; ?>" method="POST" id="ilanGaleriFotolar">
                                    <div class="ilandigerfotolar" id="yuklenen_fotolar">
                                        <ul id="list" class="uk-nestable" data-uk-nestable="{maxDepth: 1}">
                                            <?php if ($yfotolarcnt > 0) :
                                                $i = 0;
                                            ...
                                            ?>
                                                    <li class="uk-nestable-item" id="xrow_<?= $row->id; ?>" data-id="<?= $i; ?>" data-idi="<?= $row->id; ?>">
                                                        <div class="ilandetayfotos gallery">
                                                            ...
                                                            <a style="z-index: 33;" title="Döndür" id="ilandondur" class="ilanfotosil" href='javascript:window.open("<?= SITE_URL . "rotate/" . $row->id; ?>", "mywindow","status=1,toolbar=0,resizable=1,width=500,height=500");'><i class="fa fa-repeat" aria-hidden="true"></i></a>
                    ...
                    <?php
                        } // aşama 0: foto galeri ayarı sonu
                    }


if ($id !== false && $asama == 1) { // aşama 1: video sistemi başlangıcı
...
<?php
} // aşama 1: video sistemi sonu


if ($id !== false && $asama == 2 && $gayarlar->dopingler_19541956 == 1) {
    ...
    $odeme = filter_var($_GET["odeme"] ?? '', FILTER_SANITIZE_STRING);
    $customs = $_SESSION["custom"] ?? '';
    if ($fonk->bosluk_kontrol($customs) == false) {
        $custom = json_decode(base64_decode($customs), true);
    }
    $from = "insert";
?>
    <script type="text/javascript">
    ...
    <div id="doping_ekle">
        ...
        <?php } elseif ($odeme == "havale_eft" && $_SESSION["custom"] != '') { ?>
        ...
        <?php } elseif ($odeme == "paytr" && $_SESSION["custom"] != '') { ?>
            <h4 style="font-weight:bold;margin-bottom:20px;color:#be2527;font-size:18px;"><?= dil("TX543"); ?></h4>
            ...
            ?>
            <!-- SanalPos frame kodu -->
            <?php $fonk->paytr_frame($hesap->adi . " " . $hesap->soyadi, $hesap->email, $hesap->adres, $hesap->telefon, $urunadi, $ftutar, $oid); ?>
            <!-- SanalPos frame kodu end -->
            <a class="btn" href="javascript:window.history.back();"><?= dil("TX515"); ?> <i class="fa fa-arrow-left" aria-hidden="true"></i></a>

        <?php } elseif ($odeme == "iyzico" && $_SESSION["custom"] != '') { ?>
            <h4 style="font-weight:bold;margin-bottom:20px;color:#be2527;font-size:18px;"><?= dil("TX543"); ?></h4>
        ...
        <?php } elseif ($odeme == "paypal" && $_SESSION["custom"] != '') { ?>
            <h4 style="font-weight:bold;margin-bottom:20px;color:#be2527;font-size:18px;"><?= dil("TX543"); ?></h4>
            ...
            <div id="OdemeButon" style="text-align:center;margin-top:25px;"><a class="btn" href="javascript:;" onclick="ajaxHere('ajax.php?p=ilan_doping_siparis&id=<?= $snc->id; ?>&odeme=paypal&from=<?= $from; ?>','SipSonuc');"><?= dil("TX546"); ?> <i class="fa fa-angle-double-right" aria-hidden="true"></i></a></div>
...
<?php } // end of asama 2 and dopingler_19541956 == 1 ?>
...
<?php if ($id === false) { // ilan id gelmiyorsa ?>
    <script type="text/javascript">
        $(document).ready(function() {
    ...
    <?php if ($aylik_ilan_limit < 1) { ?>
        <br><br><br>
        <?= dil("TX611"); ?>
    <?php } else { ?>
        <div class="clear"></div>
        <?= str_replace(["[aylik_ilan_limit]", "[ilan_resim_limit]", "[ilan_yayin_sure]"], [$aylik_ilan_limit, $ilan_resim_limit, $ilan_yayin_sure . " " . $periyod[$ilan_yayin_periyod]], dil("TX655")); ?>
        <div class="clear"></div>
        ...
        <form action="ajax.php?p=ilan_olustur" method="POST" id="IlanOlusturForm" enctype="multipart/form-data">
            ...
            <ul class="etab">
                ...
                $dilop = [];
                $i = 1;
                $sql = $db->query("SELECT * FROM diller_19541956 WHERE kisa_adi!='" . $dil . "' ORDER BY sira ASC");
                    ...
                    <tr>
                        ...
                        <td>
                            <select onchange="yazdir();" name="mahalle" id="semt">
                                ...
                                <?php
                                if ($ilan->ilce_id != '') {
                                    $sql = $db->query("SELECT id,mahalle_adi FROM mahalle_koy WHERE ilce_id=" . $ilan->ilce_id . " ORDER BY id ASC");
                                    while ($row = $sql->fetch(PDO::FETCH_OBJ)) {
...
<?php
                    ...
                    <table>
                        <br />
                        <div class="form-group" style="text-align: center;">
                            <p><font color="red"><strong> SÖZLEŞMELİ İLANLARINIZ </strong></font><br /><font color="black">Sözleşmeli İlanlarınızı, Daha Hızlı Pazarlamak İçin,</font><br /> <font color="black">Üye Emlakçılarımız ile Paylaşabilirsiniz.
                        </div>
                        ...
                        <tr id="site_id_888_con">
                            <td><?= dil("TX27340"); ?></td>
                            <div class="form-group" style="text-align: center;">
                                <td><font color="blue"> Sözleşmeli İlanınızı, Üye Emlakçılarımız ile Paylaşmak İçin,</font>&nbsp;&nbsp;&nbsp;<font color="black">Kutunun İçine <font color="red"><strong> 100 </strong></font>Yazınız.
                        ...
                        <tr id="site_id_777_con">
                            <td><?= dil("TX27341"); ?></td>
                            <div class="form-group" style="text-align: center;">
                                <td><font color="blue"> Sözleşmeli İlanınızı, Grubunuzdaki Emlakçılar ile Paylaşmak İçin </font>&nbsp;&nbsp;&nbsp;<font color="black">Kutunun İçine <font color="red"><strong> GRUP </strong></font>Yazınız.
                    ...
                    <table>
                        <div class="form-group" style="text-align: center;">
                            <p><font color="red"><strong> KAPALI PORTFÖY İLANLARINIZ </strong></font><br /><font color="black"> Kapalı Portföy İlanlarınızı, Artık Sitenize, Girerek Saklayabilir</font><br /><font color="black">ve Dilediğiniz Zaman, Tekrar Aktive Edebilirsiniz.
                        </div>
                        ...
                        <tr id="site_id_699_con">
                            <td><?= dil("TX27342"); ?></td>
                            <div class="form-group" style="text-align: center;">
                                <td><font color="blue"> Kapalı Portföy İlanlarınızı, Üye Emlakçılarımız ile Paylaşmak için </font><br /> <font color="black"> Kutunun İçine <font color="red"><strong> 200 </strong></font>Yazınız.
                        ...
                        <tr id="site_id_700_con">
                            <td><?= dil("TX27343"); ?></td>
                            <div class="form-group" style="text-align: center;">
                                <td><font color="blue"> Kapalı Portföy İlanlarınızı, PAYLAŞMAK İSTEMEZSENİZ,</font><br /> <font color="black"> Kutucuğun İçine size verdiğimiz <font color="red"><strong> ŞİFREYİ </strong></font>Yazınız.
                        ...
                        <tr id="site_id_701_con">
                            <td><?= dil("TX27344"); ?></td>
                            <div class="form-group" style="text-align: center;">
                                <td><font color="blue"> Kapalı Portföy İlanlarınızı, GRUBUNUZDAKİ EMLAKÇILARLA PAYLAŞMAK İçin</font><br /> <font color="black"> Kutucuğun İçine,<font color="red"><strong> GRUP KODUNUZU </strong></font>Yazınız.
                    ...
                    <table>
                        <div class="form-group" style="text-align: center;">
                            <p><font color="red"><strong> EMLAK TALEPLERİNİZİ PAYLAŞIN </strong></font><br /><font color="black"> Sıtenize girdiğiniz Müşteri Talepleriniz, PORTALLARIMIZDA, Sizin Bilgileriniz ile Yayınlanır.</font><br /><font color="black">Dilediğiniz Emlak Taleplerinizi, Üye Emlakçılarımız ile de paylaşabilirsiniz.
                        </div>
                        ...
                        <tr id="site_id_702_con">
                            <td><?= dil("TX27345"); ?></td>
                            <div class="form-group" style="text-align: center;">
                                <td><font color="blue">Emlak Taleplerinizi Paylaşmak için,</font> <font color="black">Kutunun İçine <font color="red"><strong> 300 </strong></font>Yazınız.
                    ...
                    <table>
                        <div class="form-group" style="text-align: center;">
                            <p><strong><font color="blue"> İLANINIZI PORTALLARIMIZDA DA YAYINLAYABİLİRSİNİZ </font> <br /><font color="red">İSTEDİĞİNİZ ( Portalın / Portalların ) KUTUCUĞUNU İŞARETLEYİNİZ.
                        </div>
                                ...
                                <div class="ilanaciklamalar">
                                    ...
                                    ?>
                                        <div class="ilanozellik tipi_konut">
                                            ...
                                            ?>
                                                <span><label><input name="ic_ozellikler[]" type="checkbox" value="<?= $val; ?>"><?= $val; ?></label></span>
                                            <?php
                                            }
                                            ?>
                                        </div>
...
?>
    <div class="ilanozellik tipi_konut">
        ...
        ?>
            <span><label><input name="dis_ozellikler[]" type="checkbox" value="<?= $val; ?>"> <?= $val; ?></label></span>
        <?php
        }
        ?>
    </div>
...
<?php
                    ...
                    ?>
                        <div class="ilanozellik tipi_arsa">
                            ...
                            ?>
                                <span><label><input name="altyapi_ozellikler[]" type="checkbox" value="<?= $val; ?>"><?= $val; ?></label></span>
                            <?php
                            }
                            ?>
                        </div>
                    ...
                    ?>
                        <div class="ilanozellik tipi_arsa">
                            ...
                            ?>
                                <span><label><input name="konum_ozellikler[]" type="checkbox" value="<?= $val; ?>"><?= $val; ?></label></span>
                            <?php
                            }
                            ?>
                        </div>
                    ...
                    ?>
                        <div class="ilanozellik tipi_arsa">
                            ...
                            ?>
                                <span><label><input name="genel_ozellikler[]" type="checkbox" value="<?= $val; ?>"><?= $val; ?></label></span>
                            <?php
                            }
                            ?>
                        </div>
                    ...
                    ?>
                        <div class="ilanozellik tipi_arsa">
                            ...
                            ?>
                                <span><label><input name="manzara_ozellikler[]" type="checkbox" value="<?= $val; ?>"><?= $val; ?></label></span>
                            <?php
                            }
                            ?>
                        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("select[name='emlak_tipi']").change(function() {
            ...
            var select = "#yetki_bilgisi_con,#brut_metrekare_con,#site_id_888_con,#site_id_777_con,#site_id_700_con,#site_id_701_con,#site_id_661_con,#site_id_662_con,#site_id_663_con,#site_id_664_con,#site_id_665_con";
    ...
    function konut_getir(tipi) {
        if (tipi == "<?= $isyeri; ?>") {
            $("select[name=konut_tipi]").html("<?php $knttipi = dil("KNT_TIPI2"); ?><option value=''><?= dil("TX57"); ?></option><?php $parc = explode("<+>", $knttipi);
                                                                                                                                                    foreach ($parc as $val) { ?><option><?= $val; ?></option><?php } ?>");
        } else {
            $("select[name=konut_tipi]").html("<?php $knttipi = dil("KNT_TIPI"); ?><option value=''><?= dil("TX57"); ?></option><?php $parc = explode("<+>", $knttipi);
                                                                                                                                                    foreach ($parc as $val) { ?><option><?= $val; ?></option><?php } ?>");
        }
    }
