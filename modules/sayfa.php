...
<div class="content" <?= (($gayarlar->blog_sidebar == 0 && $sayfay->tipi == 1) || ($gayarlar->haberler_sidebar == 0 && $sayfay->tipi == 2) || ($gayarlar->sayfa_sidebar == 0 && $sayfay->tipi == 0)) ? 'style="float:left;"' : 'id="bigcontent"'; ?>>
...
<div class="urungiderfotolar gallery">
    <?php
    $sql = $db->query("SELECT * FROM galeri_foto WHERE site_id_555=999 AND sayfa_id=" . (int)$sayfay->id . " AND dil='" . htmlspecialchars($dil, ENT_QUOTES, 'UTF-8') . "' ORDER BY id DESC");
    while ($row = $sql->fetch(PDO::FETCH_OBJ)) {
    ?>
    <a href="uploads/<?= htmlspecialchars($row->resim, ENT_QUOTES, 'UTF-8'); ?>" rel="prettyPhoto[gallery1]"><img src="uploads/thumb/<?= htmlspecialchars($row->resim, ENT_QUOTES, 'UTF-8'); ?>" width="150" height="100"></a>
    <?php } ?>
</div>
