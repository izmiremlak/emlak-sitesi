...
if ($hesap->id == "" && $_POST) {
    ...
    $parola_tekrar = htmlspecialchars($_POST["parola_tekrar"]);
    $olusturma_tarih = $fonk->datetime();
    $ip = $fonk->IpAdresi();
    $sozlesme = intval($_POST["sozlesme"]);
    $turu = intval($_POST["turu"]);

    $unvan = htmlspecialchars($_POST["unvan"]);
    $vergi_no = htmlspecialchars($_POST["vergi_no"]);
    $vergi_dairesi = htmlspecialchars($_POST["vergi_dairesi"]);
    $adres = htmlspecialchars($_POST["adres"]);
    $tcno = intval($_POST["tcno"]);
    $tconay = 0;
    $smsonay = 0;
    $elonay = 0;
    ...
    } elseif ($fonk->bosluk_kontrol($telefon) == true && $gayarlar->sms_aktivasyon == 1) {
        die('<span class="error">' . dil("TX36") . '</span>');
    } elseif (strlen($telefon) < 10 && $gayarlar->sms_aktivasyon == 1) {
        die('<span class="error">' . dil("TX36") . '</span>');
    } elseif ($sozlesme != 1) {
    ...
    if ($turu == 0 || $turu == 2) {
        if ($fonk->bosluk_kontrol($tcno) == true && $gayarlar->tcnod == 1) {
            die('<span class="error">' . dil("TX369") . '</span>');
        } elseif ($gayarlar->tcnod == 1 && $gvn->tcNoCheck($tcno) == false) {
    ...
    $kontrol2 = $db->prepare("SELECT * FROM hesaplar WHERE site_id_555=999 AND durum=1 AND (email=? OR telefon=? OR ip=?) ");
    $kontrol2->execute([$email, $telefon, $ip]);

    if ($kontrol2->rowCount() > 0) {
        die('<span class="error">' . dil("TX39") . '</span>');
    }

    $kontrol = $db->prepare("SELECT * FROM hesaplar WHERE site_id_555=999 AND (email=? OR telefon=?) ");
    $kontrol->execute([$email, $telefon]);
    ...
    if ($sms_onay == 1) {
        $gonay = intval($_GET["gonay"]);
        if ($gonay == 1) {
            $scode = intval($_POST["scode"]);

            if ($scode == '') {
                die('<span class="error">' . dil("TX374") . '</span>');
            }

            $sorgula = $db->prepare("SELECT id FROM sgonderiler_19541956 WHERE no=? AND scode=? AND tarih=?");
            $sorgula->execute([$telefon, $scode, $fonk->this_date()]);
            $hak = $_SESSION["hak"];

            if ($hak >= 3) {
                die('<span class="error">' . dil("TX375") . '</span>');
            } elseif ($sorgula->rowCount() > 0) {
                $del = $db->prepare("DELETE FROM sgonderiler_19541956 WHERE scode=?");
                $del->execute([$scode]);
                unset($_SESSION["hak"]);
            } else {
                $hak = $hak + 1;
            ...
            $cachesorg = $db->prepare("SELECT id FROM sgonderiler_19541956 WHERE ip=? AND tarih=?");
            $cachesorg->execute([$ip, $fonk->this_date()]);
            ...
            $fonk->bildirim_gonder([$scode, date("d.m.Y H:i")], "uyelik_sms_onay_kodu", $email, $telefon);

            $cacins = $db->prepare("INSERT INTO sgonderiler SET ip=?, tarih=?, scode=?, no=? ");
            $cacins->execute([$ip, $fonk->this_date(), $scode, $telefon]);
            $_SESSION["hak"] = 0;
            ?>
            <script>
    ...
    try {
        $olustur = $db->prepare("INSERT INTO hesaplar SET site_id_888=XXX, site_id_777=XXX, site_id_699=XXX, site_id_700=XXX, site_id_701=XXX, site_id_702=XXX, site_id_555=555, site_id_450=450, site_id_444=444, site_id_333=333, site_id_335=335, site_id_334=334, site_id_306=306, site_id_222=222, site_id_111=111, adi=?, soyadi=?, email=?, parola=?, olusturma_tarih=?, telefon=?, ip=?, mail_izin=?, sms_izin=?, turu=?, nick_adi=?, unvan=?, vergi_no=?, vergi_dairesi=?, tcno=?, adres=?, aylik_ilan_limit=?, ilan_resim_limit=?, ilan_yayin_sure=?, ilan_yayin_periyod=?, danisman_limit=?");
        $olustur->execute([$adi, $soyadi, $email, $parola, $olusturma_tarih, $telefon, $ip, $tconay, $smsonay, $turu, $nick_adi, $unvan, $vergi_no, $vergi_dairesi, $tcno, $adres, $aylik_ilan_limit, $ilan_resim_limit, $ilan_yayin_sure, $ilan_yayin_periyod, $danisman_limit]);
    ...
    if ($lid != "" || $lid != 0) {
        ...
        $baskasi->execute([$nick_adi, $lid]);
        if ($baskasi->rowCount() > 0) {
            $nick_adi .= "-" . $lid;
            $nup = $db->prepare("UPDATE hesaplar SET nick_adi=? WHERE site_id_555=999 AND id=?");
            $nup->execute([$nick_adi, $lid]);
        }

        $db->query("UPDATE hesaplar SET login_secret='$secret' WHERE site_id_555=999 AND id=$lid");
        ?>
        <script>
        ...
        <?php
        $fonk->bildirim_gonder([$adsoyad, $email, $parola, SITE_URL . "hesabim"], "kaydol", $email, $telefon);

        $gt = htmlspecialchars($_SERVER["HTTP_REFERER"]);
