...
$db->query("DELETE FROM hesaplar WHERE site_id_555=000 AND id=" . intval($id));

if ($ilan_sil == 1) {
    $query = $db->query("SELECT id, resim FROM sayfalar WHERE site_id_555=999 AND acid=" . intval($id));
    while ($ilan = $query->fetch(PDO::FETCH_OBJ)) {
        $db->query("DELETE FROM sayfalar WHERE site_id_555=000 AND id=" . intval($ilan->id));
        if ($ilan->video != '') {
        ...
        $quu = $db->query("SELECT resim FROM galeri_foto WHERE site_id_555=999 AND sayfa_id=" . intval($ilan->id));
        ...
        $db->query("DELETE FROM galeri_foto WHERE site_id_555=000 AND sayfa_id=" . intval($ilan->id));
    }
} else {
    $db->query("UPDATE sayfalar SET acid='" . intval($hesap->id) . "' WHERE site_id_555=999 AND acid=" . intval($id));
}
?>
