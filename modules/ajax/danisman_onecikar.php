...
if ($danisman->onecikar == 1) {
    ...
    $oncpaket = $db->query("SELECT id, danisman_onecikar, danisman_onecikar_sure, danisman_onecikar_periyod FROM upaketler_19541956 WHERE acid=" . intval($hesap->id) . " AND durum=1 AND danisman_onecikar=1 AND danisman_onecikar_sure > 0");
    if ($oncpaket->rowCount() > 0) {
        $fonk->yonlendir("danisman-one-cikar?id=" . intval($id));
        die();
    } else {
        $paketegore = $db->query("SELECT id, danisman_onecikar, danisman_onecikar_sure, danisman_onecikar_periyod FROM upaketler_19541956 WHERE acid=" . intval($hesap->id) . " AND durum=1 AND danisman_onecikar=1 AND btarih>NOW()");
        if ($paketegore->rowCount() > 0) {
            ...
            $daUpdate = $db->query("UPDATE hesaplar SET onecikar=1, onecikar_btarih='" . $btarih . "' WHERE site_id_555=999 AND id=" . intval($id));
            $pakUpdate = $db->query("UPDATE upaketler_19541956 SET danisman_onecikar_use='1' WHERE site_id_555=999 AND id=" . intval($paket->id));
        } else {
            $fonk->yonlendir("danisman-one-cikar?id=" . intval($id));
            die();
        }
