...
$acc = $db->query("SELECT id, kid FROM hesaplar WHERE site_id_555=999 AND id=" . intval($ilan->acid))->fetch(PDO::FETCH_OBJ);
$kid = $acc->kid;
...
// Çoklu ilan kontrolü
$multi = $db->query("SELECT id, ilan_no FROM sayfalar WHERE site_id_555=999 AND ilan_no=" . intval($ilan->ilan_no) . " ORDER BY id ASC");
$multict = $multi->rowCount();
$multif = $multi->fetch(PDO::FETCH_OBJ);
$multidids = $db->query("SELECT GROUP_CONCAT(id SEPARATOR ',') AS ids FROM sayfalar WHERE site_id_555=999 AND ilan_no=" . intval($ilan->ilan_no))->fetch(PDO::FETCH_OBJ)->ids;
