...
if ($odeme == "havale_eft") {
    ...
    try {
        $query = $db->prepare("INSERT INTO upaketler_19541956 SET acid=?, pid=?, adi=?, tutar=?, durum=?, odeme_yontemi=?, tarih=?, btarih=?, sure=?, periyod=?, aylik_ilan_limit=?, ilan_resim_limit=?, ilan_yayin_suresi=?, ilan_video_limit=?");
        $query->execute(array($hesap_id, $paket->id, $paket->baslik, $secilen["tutar"], $durum, $odeme_yontemi, $tarih, $btarih, $secilen["sure"], $secilen["periyod"], $paket->aylik_ilan_limit, $paket->ilan_resim_limit, $paket->ilan_yayin_suresi, $paket->ilan_video_limit));
