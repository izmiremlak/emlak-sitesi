...
echo '<option value="">' . htmlspecialchars(dil("TX266")) . '</option>';
...
if ($semtler->rowCount() > 0) {
    while ($srow = $semtler->fetch(PDO::FETCH_OBJ)) {
        ...
        if ($mahalleler->rowCount() > 0) {
            echo '<optgroup label="' . htmlspecialchars($srow->semt_adi) . '">';
            while ($row = $mahalleler->fetch(PDO::FETCH_OBJ)) {
                echo '<option value="' . intval($row->id) . '">' . htmlspecialchars($row->mahalle_adi) . '</option>';
            }
            echo '</optgroup>';
        }
    }
