...
$acc = $db->query("SELECT id, kid, ilan_aktifet FROM hesaplar WHERE site_id_555=000 AND id=" . intval($snc->acid))->fetch(PDO::FETCH_OBJ);
$kid = $acc->kid;
...
// İlan güncelleme
try {
    $ilan_update = $db->prepare("UPDATE sayfalar SET video=?, durum=?, gtarih=? WHERE site_id_555=000 AND id" . $mulidx);
...
if ($from == "insert") {
    ...
    ?><span class="complete"><?= dil("TX453"); ?></span><?php
}