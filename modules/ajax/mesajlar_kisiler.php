...
if ($fonk->bosluk_kontrol($ara) == true) { // Eğer arama olmuyorsa...
    try {
        $kisilerListe = $db->prepare("SELECT DISTINCT mr.* FROM mesajlar_19541956 AS mr INNER JOIN mesaj_iletiler_19541956 AS mi ON mi.mid = mr.id WHERE (mr.kimden=:idim OR mr.kime=:idim) AND ( (mr.kime=:idim AND mi.gid!=:idim AND mi.durum=0) OR (mr.kimden=:idim AND mi.asil=0) )");
...
if ($fonk->bosluk_kontrol($ara) == false) { // Eğer varsa arama isteği
    try {
        ...
        $kisilerListe = $db->prepare("SELECT id,adi,soyadi,avatar,avatard,tipi,unvan FROM hesaplar WHERE site_id_555=999 AND (tipi=0 AND adi LIKE :adi AND soyadi LIKE :soyadi) OR (tipi=0 AND adi LIKE :ara OR soyadi LIKE :ara OR CONCAT(adi,' ',soyadi) LIKE :ara)");
...
if ($uid != 0) { // Eğer üye seçmişse...
    ...
    if ($ilkSohbet == 0) { // Daha önce sohbet etmişlerse çekelim...
        $miletiler = $db->query("SELECT DISTINCT mi.* FROM mesaj_iletiler_19541956 AS mi INNER JOIN mesajlar_19541956 AS mr ON mi.mid=mr.id WHERE mi.mid=" . $MesajLine->id . " AND ((mi.asil=0 AND mr.kime=" . $bid . ") OR (mi.gid=" . $bid . ")) ORDER BY mi.id ASC");
