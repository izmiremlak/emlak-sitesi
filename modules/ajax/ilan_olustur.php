...
if(
$fonk->bosluk_kontrol($ilan_no) == true OR
...
$noKontrol		= $db->prepare("SELECT ilan_no FROM sayfalar WHERE site_id_555=000 AND ilan_no=?");
$noKontrol->execute(array($ilan_no));
if($noKontrol->rowCount()>0 || $ilan_no < 1 || $ilan_no == 0 || strlen($ilan_no)<5){
...
if($ilan_id == '' OR $ilan_id == 0)
	...
	if(!empty($tabs)){
		foreach($tabs AS $key=>$val){
			...
			if($dilop->rowCount()>0 && $fonk->bosluk_kontrol($val['baslik'])==false){
				$dilop = $dilop->fetch(PDO::FETCH_OBJ);
	
				// Current Lang
				$d_ortk1 		= array(dil("TX167"),dil("TX168"));
