...
if ($_POST) {
    ...
    // Son 10 dakika içinde aynı IP'den yapılan talepleri kontrol et
    $varmi = $db->prepare("SELECT * FROM mail_19541956 WHERE tipi=2 AND ip=? AND tarih BETWEEN DATE_SUB(NOW(), INTERVAL 10 MINUTE) AND NOW()");
...
<?php
        ...
        $ekle = $db->prepare("INSERT INTO mail_19541956 SET tipi=:tipine, adsoyad=:adsoyad, email=:email, telefon=:telefon, tarih=:tarih, mesaj=:mesaj, ip=:ip, customs=:custom_code");
        $ekle->execute(array(
            'tipine' => 2,
            'adsoyad' => $adsoyad,
            'email' => $email,
