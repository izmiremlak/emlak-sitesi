...
$qry = $pagent->sql_query("SELECT * FROM sayfalar WHERE site_id_555=888 AND tipi=1 AND dil='" . htmlspecialchars($dil, ENT_QUOTES, 'UTF-8') . "' ORDER BY id DESC", $gvn->rakam($_GET["git"]), 5);
...
if ($query->rowCount() > 0) {
    ...
    <div class="listeleme">
        ...
        <div class="listeicerik">
            ...
            <p><?= htmlspecialchars($fonk->kisalt($icerik, 0, 335), ENT_QUOTES, 'UTF-8'); ?><?= (strlen($icerik) > 335) ? '...' : ''; ?> <strong><a class="detaylink" href="<?= htmlspecialchars($linkx, ENT_QUOTES, 'UTF-8'); ?>"><?= htmlspecialchars(dil("TX137"), ENT_QUOTES, 'UTF-8'); ?></a></strong></p>
        </div>
    </div>
    <?php
    }
