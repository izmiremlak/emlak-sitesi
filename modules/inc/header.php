...
<div class="header" style="top: auto;">
    ...
    <div id="wrapper">
        <?php if($hesap->id == ""){ ?>
            ...
            <?php if($gayarlar->uyelik == 1){ ?><a class="gonderbtn" id="ilanverbtn" href="hesap-olustur"><i class="fa fa-user-plus" aria-hidden="true"></i> <?=dil("TX125");?></a><?php } ?>
        <?php } else { ?>
            <a class="gonderbtn" id="ilanverbtn" href="logout.php"><i class="fa fa-sign-in" aria-hidden="true"></i> <?=dil("TX112");?></a>
            <a class="gonderbtn" id="ilanverbtn" href="uye-paneli"><span style="margin-left: -29px; position: absolute; margin-top: -15px;display:none" class="msjvar mbildirim">0</span><i class="fa fa-user-circle" aria-hidden="true"></i> <?=dil("TX113");?></a>
        <?php } ?>
        ...
        <?php
        $diller = $db->query("SELECT kisa_adi,gosterim_adi FROM diller_501 WHERE durum=0 ORDER BY sira ASC");
    ...
    <?php if ($p == '' AND $gayarlar->urun_siparis == 1) { ?>
        <div class="homearama">
            <div id="wrapper">
                    ...
                    <form action="ajax.php?p=ilanlar" method="POST" id="IlanlarAramaFormHeader">
                        ...
                        ?>
                            <select name="emlak_durum" id="leftradius">
                                ...
                                foreach ($parc as $val) {
                                ?><option <?=($val == $emlak_durum) ? 'selected' : '';?>><?=$val;?></option><?php
                                }
                                ?>
                            </select>
                        <?php } ?>
                        ...
                        ?>
                            <select name="emlak_tipi">
                                ...
                                $parc = explode("<+>", $emlktp);
                                foreach ($parc as $val) {
                                ?><option <?=($val == $emlak_tipi) ? 'selected' : '';?>><?=$val;?></option><?php
                                }
                                ?>
                            </select>
                        <?php } ?>
                        ...
                        <select name="il" id="hdil" onchange="ajaxHere('ajax.php?p=ilce_getir&varsa=1&il_id='+this.options[this.selectedIndex].value,'hdilce');">
                            ...
                            if ($ulkelerc < 2) {
                                ...
                                ?><option value="<?=$row->id;?>" <?=($row->id == $il) ? 'selected' : '';?>><?=$row->il_adi;?></option><?php
                                }
                            }
                            ?>
                        </select>
