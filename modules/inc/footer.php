...
try {
    ...
    while($row = $sql->fetch(PDO::FETCH_OBJ)){
        echo '<a href="' . htmlspecialchars($row->website, ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($row->adi, ENT_QUOTES, 'UTF-8') . '</a>';
    }
...
<?php if($dayarlar->slogan3 != '') {?><p><?=$dayarlar->slogan3;?></p><?php } ?>
<?php if($dayarlar->telefon != '') {?><h4><span><?=dil("TX76");?> </span><strong><a style="color:white;" href="tel:<?=$dayarlar->telefon;?>"><?=$dayarlar->telefon;?></a></strong></h4><?php } ?>
<?php if($dayarlar->faks != '') {?><h4><span><?=dil("TX77");?> </span><strong><?=$dayarlar->faks;?></strong></h4><?php } ?>
<?php if($dayarlar->email != '') {?><h4><span><?=dil("TX78");?> </span><strong><?=$dayarlar->email;?></strong></h4><?php } ?>
<?php if($dayarlar->adres != '') {?><h5><span><?=dil("TX79");?> </span><?=$dayarlar->adres;?></h5><?php } ?>
</div>

<?php
function getPageUrl($pageId) {
    global $dayarlar, $db;
    $page = $db->query("SELECT id,url FROM sayfalar WHERE site_id_555=888 AND id=" . intval($pageId))->fetch(PDO::FETCH_OBJ);
    return ($dayarlar->permalink == 'Evet') ? $page->url . '.html' : 'index.php?p=sayfa&id=' . $page->id;
}

$sayfa_urls = [];*********************
for ($i = 1; $i <= 7; $i++) {
    $pageId = $dayarlar->{'foot_sayfa' . $i};
    $sayfa_urls[$i] = $pageId != 0 ? getPageUrl($pageId) : $dayarlar->{'foot_link' . $i};
}
?>
<div class="footblok" id="footlinks">
<h3><?=dil("TX80");?></h3>
<?php for ($i = 1; $i <= 7; $i++) { ?>
    <a href="<?=$sayfa_urls[$i];?>"><?=$dayarlar->{'foot_text' . $i};?></a>
<?php } ?>
</div>
...
<?php if($dayarlar->facebook != '' OR $dayarlar->instagram != '' OR $dayarlar->twitter != ''){ ?>
<div class="headsosyal">
<?php if($dayarlar->facebook != ''){ ?><a target="_blank" href="<?=$dayarlar->facebook;?>"><i class="fa fa-facebook" aria-hidden="true"></i></a><?php } ?>
<?php if($dayarlar->twitter != ''){ ?><a target="_blank" href="<?=$dayarlar->twitter;?>"><i class="fa fa-twitter" aria-hidden="true"></i></a><?php } ?>
<?php if($dayarlar->instagram != ''){ ?><a target="_blank" href="<?=$dayarlar->instagram;?>"><i class="fa fa-instagram" aria-hidden="true"></i></a><?php } ?>
</div>
<?php } ?>
</div>
</div>
...
<?php if($p == 'uye_paneli' AND $rd == "danismanlar"){ ?>
...
<?php if(($p == 'sayfa' AND $sayfay->tipi == 4) || ($p == 'sayfa' AND $sayfay->tipi == 5)){ ?>
...
<?php } ?>
<?php } elseif($p == 'uye_paneli' && $rd == 'mesajlar' && $gayarlar->anlik_sohbet==1){ ?>
<script type="text/javascript">
        ...
        if(uid != 0 && data.sohbet != undefined){
            ...
            if(ae.val() != uid){
                $("#MesajlasmaContent").fadeOut(100, function(){
                    ...
                    if(data.sohbet.engelbutonu == 1){
                        $("#EngelButon").hide(10);
                    } else {
                        $("#EngelButon").show(10);
                    }
                    if(data.sohbet.benEngel == 1){
                        $("#EngelButon").html('<i class="fa fa-ban" aria-hidden="true"></i> <?=dil("TX402");?>');
                    } else {
                        $("#EngelButon").html('<i class="fa fa-ban" aria-hidden="true"></i> <?=dil("TX403");?>');
                    }
                    if(data.sohbet.grsmesilbuton == 1){
                        $("#GorusmeSilButon").hide(10);
                    } else {
                        $("#GorusmeSilButon").show(10);
                    }
...
<script type="text/javascript">
<?php if($rd != 'mesajlar' && $hesap->id != '' && $gayarlar->anlik_sohbet==1){ ?>
...
<?php if($p == "uye_paneli" && ($rd == "dopinglerim" || $rd == "paketlerim" || $rd == "")){ ?>
...
<?php if($p == 'uye_paneli' && ($_GET["rd"] == 'ilan_duzenle' || $_GET["rd"] == 'ilan_olustur')){ ?>
...
<?php echo $dayarlar->analytics; ?>
<?php echo $dayarlar->embed; ?>
