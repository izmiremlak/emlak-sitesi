...
$baslangic	= 0;
$bitis		= 20;
$sql		= $db->query("SELECT DISTINCT t1.ilan_no,t1.id,t1.url,t1.fiyat,t1.baslik,t1.resim,t1.il_id,t1.ilce_id,t1.emlak_durum,t1.pbirim,t1.emlak_tipi FROM sayfalar AS t1 LEFT JOIN dopingler_19541956 AS t2 ON t2.ilan_id=t1.id AND t2.durum=1 AND t2.did=3 WHERE t2.btarih>NOW() AND t1.site_id_555=999 AND t1.tipi=4 AND t1.durum=1 ORDER BY RAND() LIMIT ".$baslangic.",".$bitis);
if($sql->rowCount()>0){
?><!-- Öne Çıkan İlanlar -->
<div id="wrapper"> 
...
<div class="list_carousel">
		...
		$row_lang = $db->query("SELECT ilan_no,id,url,fiyat,baslik,resim,il_id,ilce_id,emlak_durum,pbirim,emlak_tipi FROM sayfalar WHERE site_id_555=999 AND ilan_no=".$row->ilan_no." AND dil='".$dil."' ");
