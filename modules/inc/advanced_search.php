<div class="altbaslik"> 
   <h4 id="mobileMenuToggle">
        <i class="fa fa-search" aria-hidden="true"></i> <strong><?=dil("TX51");?></strong>
    </h4>
...
<input name="min_bina_kat_sayisi" type="text" value="<?=($min_bina_kat_sayisi != 0) ? $min_bina_kat_sayisi : '';?>" id="yariminpt" placeholder="<?=dil("TX64");?>" class="konut">

<input name="max_bina_kat_sayisi" type="text" value="<?=($max_bina_kat_sayisi != 0) ? $max_bina_kat_sayisi : '';?>" id="yariminpt" placeholder="<?=dil("TX65");?>" class="konut">
...
<?php
$kaks_emsaller = dil("KAKS_EMSAL");
if($kaks_emsaller != ''){
?>
<select name="kaks_emsal" class="arsa">
<option value=""><?=dil("TX331");?></option>
<?php
$exps = explode("<+>",$kaks_emsaller);
foreach ($exps as $key => $value){
	?><option <?php echo $kaks_emsal == $value ? 'selected' : ''; ?>><?php echo $value; ?></option><?php
}
?>
</select>
<?php
...
<?php
$gabariler = dil("GABARI");
if($gabariler != ''){
?>
<select name="gabari" class="arsa">
<option value=""><?=dil("TX332");?></option>
<?php
$exps = explode("<+>",$gabariler);
foreach ($exps as $key => $value){
	?><option <?php echo $gabari == $value ? 'selected' : ''; ?>><?php echo $value; ?></option><?php
}
?>
</select>
<?php
...
<?php
$imar_durumlari = dil("IMAR_DURUM");
if($imar_durumlari != ''){
?>
<select name="imar_durum" class="arsa">
<option value=""><?=dil("TX682");?></option>
<?php
$exps = explode("<+>",$imar_durumlari);
foreach ($exps as $key => $value){
	?><option <?php echo $imar_durum == $value ? 'selected' : ''; ?>><?php echo $value; ?></option><?php
}
?>
</select>
<?php
...
<?php
$tapu_durumlari = dil("TAPU_DRM");
if($tapu_durumlari != ''){
?>
<select name="tapu_durumu" class="arsa">
<option value=""><?=dil("TX333");?></option>
<?php
$exps = explode("<+>",$tapu_durumlari);
foreach ($exps as $key => $value){
	?><option <?php echo $tapu_durumu == $value ? 'selected' : ''; ?>><?php echo $value; ?></option><?php
}
?>
</select>
<?php
...
<option value=""><?=dil("TX334");?></option>
<option<?=($katk == dil("TX167")) ? ' selected' : '';?>><?=dil("TX167");?></option>
<option<?=($katk == dil("TX168")) ? ' selected' : '';?>><?=dil("TX168");?></option>
</select>
...
<option value=""><?=dil("TX335");?></option>
<?php
foreach($exp as $row){
?><option<?=($krediu == $row) ? ' selected' : '';?>><?=$row;?></option><?
}
?>
</select>
<?}?>
...
<label for="resimli" class="checkbox-custom-label" style="margin-top:7px;"><span class="checktext"><?=dil("TX613");?></span></label>
