<div class="headerbg" <?=($gayarlar->belgeler_resim  != '') ? 'style="background-image: url(uploads/'.$gayarlar->belgeler_resim.');"' : ''; ?>>
<div id="wrapper">
<div class="headtitle">
<h1><?=dil("TX230");?></h1>
<div class="sayfayolu">
<span><?=dil("TX231");?></span>
</div>
</div>

</div>
<div class="headerwhite"></div>
</div>

<div id="wrapper">

<div class="uyepanel">

<div class="content">

<?php
if($gayarlar->reklamlar == 1){ // Eğer reklamlar aktif ise...
$detect 	= (!isset($detect)) ? new Mobile_Detect : $detect;
$rtipi		= 8;
$reklamlar	= $db->query("SELECT id FROM reklamlar_19561954 WHERE tipi={$rtipi} AND durum=0 AND (btarih > NOW() OR suresiz=1)");
$rcount		= $reklamlar->rowCount();
$order		= ($rcount>1) ? "ORDER BY RAND()" : "ORDER BY id DESC";
$reklam		= $db->query("SELECT * FROM reklamlar_19561954 WHERE tipi={$rtipi} AND (btarih > NOW() OR suresiz=1) ".$order." LIMIT 0,1")->fetch(PDO::FETCH_OBJ);
if($rcount>0){
?><!-- 728 x 90 Reklam Alanı -->
<div class="ad728home">
<?=($detect->isMobile() || $detect->isTablet()) ? $reklam->mobil_kodu : $reklam->kodu;?>
</div>
<!-- 728 x 90 Reklam Alanı END-->
<? }} // Eğer reklamlar aktif ise... ?>

<div class="uyedetay">
<div class="uyeolgirisyap">
<h4 class="uyepaneltitle"><?=dil("TX230");?></h4>


<?php

if($hesap->turu == 1){
$dids			= $db->query("SELECT kid,id,GROUP_CONCAT(id SEPARATOR ',') AS danismanlar_19541956 FROM hesaplar WHERE site_id_555=999 AND kid=".$hesap->id)->fetch(PDO::FETCH_OBJ);
$danismanlar	= $dids->danismanlar;
$acids			= ($danismanlar == '') ? $hesap->id : $hesap->id.','.$danismanlar;
}else{
$acids			= $hesap->id;
}

$git		= $gvn->zrakam($_GET["git"]);
$qry		= $pagent->sql_query("SELECT DISTINCT t1.id,t1.url,t1.tarih,t1.baslik,t1.durum,t1.ilan_no,t1.hit,t1.resim FROM sayfalar AS t1 LEFT JOIN dopingler_19541956 AS t2 ON t2.ilan_id=t1.id AND t2.durum=1 WHERE (t1.btarih>NOW() OR t2.btarih>NOW()) AND t1.site_id_555=999 AND t1.tipi=4 AND t1.ekleme=1 AND (t1.durum=1 OR t1.durum=0) AND t1.acid IN(".$acids.") ORDER BY t1.id DESC",$git,50000);
$query 		= $db->query($qry['sql']);
$adet		= $qry['toplam'];
?>

<?php
if($adet > 0 ){
?>
<div id="hidden_result" style="display:none"></div>
<table width="100%" border="0" id="datatable">
   <thead style="background:#ebebeb;">  
                <tr>  
                    <th align="center"><strong><?=dil("TX232");?></strong></th>  
                    <th align="left"><strong><?=dil("TX233");?></strong></th>  
                    <th  id="mobtd" align="center"><strong><?=dil("TX234");?></strong></th> 
                    <th align="center"><strong><?=dil("TX235");?></strong></th> 
                </tr>  
            </thead> 
  
  
<?php
while($row	= $query->fetch(PDO::FETCH_OBJ)){
$ilink		= ($dayarlar->permalink == 'Evet') ? $row->url.'.html' : 'index.php?p=sayfa&id='.$row->id;
$ilan_tarih	= date("d.m.Y",strtotime($row->tarih));
?>
  <tr id="row_<?=$row->id;?>">
    <td><img src="https://www.turkiyeemlaksitesi.com.tr/uploads/thumb/<?=$row->resim;?>" width="100" height="75"/></td>
    <td><a href="<?=$ilink;?>"><strong><?php echo $row->baslik;?></strong></a><br>
    <span class="ilantarih"><?=dil("TX236");?> <?=$ilan_tarih;?></span>
    <span class="ilantarih"><?php if($row->durum == 1 OR $row->durum == 3){ ?><?=dil("TX315");?> <?php echo $row->hit;?><?php } ?> </span>
    <span class="ilantarih"><?php echo dil("TX140"); ?>: <?php echo $row->ilan_no; ?></span>
    </td>
    <td align="center"  id="mobtd">
	<?php
	if($row->durum == 0){
	?><span style="color:red;font-weight:bold;"><?=dil("TX238");?></span><?
	}elseif($row->durum == 1){
	?><span style="color:green;font-weight:bold;"><?=dil("TX239");?></span><?
	}elseif($row->durum == 2){
	?><span style="color:green;font-weight:bold;"><?=dil("TX240");?></span><?
	}elseif($row->durum == 3){
	?><span style="color:orange;font-weight:bold;"><?=dil("TX241");?></span><?
  }
	?>
	</td>
    <td width="15%" align="center">
	
    <a title="Düzenle" class="uyeilankontrolbtn" href="uye-paneli?rd=ilan_duzenle&id=<?=$row->id;?>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
    <? if($row->durum == 1){ ?>
	<a title="Pasif Yap" class="uyeilankontrolbtn" href="javascript:;" onclick="ajaxHere('ajax.php?p=ilan_pasif&id=<?=$row->id;?>','hidden_result');"><i class="fa fa-power-off" aria-hidden="true"></i></a>
	<? } ?>
    <a title="Sil" class="uyeilankontrolbtn" href="javascript:;" onclick="ajaxHere('ajax.php?p=ilan_sil&id=<?=$row->id;?>','hidden_result');"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
    </td>
  </tr>
  <? } ?>
  
  </table>


<? /*<div class="clear"></div>
<div class="sayfalama">
<?php echo $pagent->listele('aktif-ilanlar?git=',$git,$qry['basdan'],$qry['kadar'],'class="sayfalama-active"',$query); ?>
</div> */ ?>

<? }else{ ?> 
<h4 style="text-align:center;margin-top:60px;"><?=dil("TX242");?></h4>
<? } ?>



</div>
</div>
</div>

<div class="sidebar">
<? include THEME_DIR."inc/uyepanel_sidebar.php"; ?>
</div>

<div class="clear"></div>

</div>

</div>


