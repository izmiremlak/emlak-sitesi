...
if($id!=''){
$snc		= $db->prepare("SELECT * FROM sayfalar WHERE site_id_555=000 AND id=? AND tipi=4");
...
header("Location:ilan-olustur"); die();
}
...
<form action="ajax.php?p=galeri_foto_guncelle&ilan_id=<?=$snc->id;?>&from=insert<? echo($yfotolarcnt>0) ? '&photos=1' : '&photos=0';?>" method="POST" id="ilanGaleriFotolar">
...
<select onchange="ajaxHere('ajax.php?p=mahalle_getir&ilce_id='+this.options[this.selectedIndex].value,'semt'),yazdir();" name="ilce" id="ilce">
  ...
  <tr>
	...
	<select onchange="yazdir();" name="mahalle" id="semt">
		...
		while($row	= $sql->fetch(PDO::FETCH_OBJ)){
		?><option value="<?=$row->id;?>" <?=($row->id == $ilan->mahalle_id) ? 'selected' : '';?>><?=$row->mahalle_adi;?></option><?
		}
		}
		?>
</select>
