...
<ul class="tab">
  <li><a href="javascript:void(0)" class="tablinks" onclick="openCity(event, 'uyelik_bilgileri')" <? echo ($_GET["goto"]=="") ? 'id="defaultOpen"' : '';?>><?=dil("TX609");?></a></li>
  <li><a href="javascript:void(0)" class="tablinks" onclick="openCity(event, 'ilanlari')" <? echo ($_GET["goto"]=="ilanlari") ? 'id="defaultOpen"' : '';?>><?=dil("TX504");?></a></li>
  <li><a href="javascript:void(0)" class="tablinks" onclick="openCity(event, 'ilanlari2')" <? echo ($_GET["goto"]=="ilanlari2") ? 'id="defaultOpen"' : '';?>><?=dil("TX507");?></a></li>
</ul>
...
<img  src="<?=($snc->avatar == '') ? 'https://www.turkiyeemlaksitesi.com.tr/uploads/default-avatar.png' : 'https://www.turkiyeemlaksitesi.com.tr/uploads/thumb/'.$snc->avatar;?>" id="avatar_image" />
  ...
  <tr>
    ...
    <a target="_blank" href="profil/<?=($snc->nick_adi == '') ? $snc->id : $snc->nick_adi;?>"><? echo SITE_URL; ?>profil/<?=($snc->nick_adi == '') ? $snc->id : $snc->nick_adi;?></a>
    </div>
    </td>
  </tr>
  ...
  <tr>
	...
	<tr style="font-size:13px;display:none">
       ...
       <input id="telefond_check" class="checkbox-custom" name="telefond" value="1" type="checkbox" <?php echo ($snc->telefond == 1) ? 'checked' : '';?> style="width:100px;">
	   <label for="telefond_check" class="checkbox-custom-label"><span class="checktext"><?=dil("TX386");?></span></label>
       <div class="clear" style="margin-bottom:5px;"></div>
	   
	   <input id="sabittelefond_check" class="checkbox-custom" name="sabittelefond" value="1" type="checkbox" <?php echo ($snc->sabittelefond == 1) ? 'checked' : '';?> style="width:100px;">
	   <label for="sabittelefond_check" class="checkbox-custom-label"><span class="checktext"><?=dil("TX387");?></span></label>
       <div class="clear" style="margin-bottom:5px;"></div>
	   
	   <input id="epostad_check" class="checkbox-custom" name="epostad" value="1" type="checkbox" <?php echo ($snc->epostad == 1) ? 'checked' : '';?> style="width:100px;">
	   <label for="epostad_check" class="checkbox-custom-label"><span class="checktext"><?=dil("TX388");?></span></label>
       <div class="clear" style="margin-bottom:5px;"></div>
	   
	   <input id="avatard_check" class="checkbox-custom" name="avatard" value="1" type="checkbox" <?php echo ($snc->avatard == 1) ? 'checked' : '';?> style="width:100px;">
       ...
       <h5 style="margin-bottom:7px;margin-top:10px;"><strong><?=dil("TX399");?></strong></h5>
        <input id="checkbox-6" class="checkbox-custom" name="mail_izin" value="1" type="checkbox" <?php echo ($snc->mail_izin == 1) ? 'checked' : '';?> style="width:100px;">
	   <label for="checkbox-6" class="checkbox-custom-label"><span class="checktext"><?=dil("TX251");?></span></label>
       <div class="clear" style="margin-bottom:5px;"></div>
       <input id="checkbox-7" class="checkbox-custom" name="sms_izin" value="1" type="checkbox" <?php echo ($snc->sms_izin == 1) ? 'checked' : '';?> style="width:100px;">
...
$qry		= $pagent->sql_query("SELECT * FROM sayfalar WHERE site_id_555=999 AND dil='".$dil."' AND ekleme=1 AND tipi=4 AND (durum=1 OR durum=0) AND acid=".$snc->id." ORDER BY id DESC",$git,6);
...
?>
  <tr id="row_<?=$row->id;?>">
	...
	<?php
	if($row->durum == 0){
	?><span style="color:red;font-weight:bold;"><?=dil("TX238");?></span><?
	}elseif($row->durum == 1){
	?><span style="color:green;font-weight:bold;"><?=dil("TX239");?></span><?
	}elseif($row->durum == 2){
	?><span style="color:green;font-weight:bold;"><?=dil("TX240");?></span><?
	}elseif($row->durum == 3){
...
$qry		= $pagent->sql_query("SELECT * FROM sayfalar WHERE site_id_555=999 AND dil='".$dil."' AND ekleme=1 AND tipi=4 AND (durum=2 OR durum=3) AND acid=".$snc->id." ORDER BY id DESC",$git2,6);
$query 		= $db->query($qry['sql']);
$adet		= $qry['toplam'];
?>
