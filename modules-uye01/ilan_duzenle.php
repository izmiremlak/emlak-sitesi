...
if($kontrol->rowCount() < 1){
	header("Location: aktif-ilanlar"); die();
}
...
$dilx			= $db->query("SELECT * FROM diller_19541956 WHERE kisa_adi='".$snc->dil."' ")->fetch(PDO::FETCH_OBJ);
...
$buay				= date("Y-m");
$aylik_ilan_limit	-= $db->query("SELECT tarih,id FROM sayfalar WHERE site_id_555=000 AND ekleme=1 AND tipi=4 ".$paketegore."AND acid IN(".$acids.") AND tarih LIKE '%".$buay."%' ")->rowCount();
...
<ul class="tab">
  <li><a href="javascript:void(0)" class="tablinks" onclick="openCity(event, 'ilandetaylari')" <? echo ($_GET["goto"]=="") ? 'id="defaultOpen"' : '';?>><i class="fa fa-info" aria-hidden="true"></i> <?=dil("TX345");?></a></li>

  <?php if($multict>0 && $snc->id == $multif->id){?>
  
  <li><a href="javascript:void(0)" class="tablinks" onclick="openCity(event, 'ilanfotolari')" <? echo ($_GET["goto"]=="photos") ? 'id="defaultOpen"' : '';?>><i class="fa fa-camera" aria-hidden="true"></i> <?=dil("TX346");?></a></li>

  <li><a href="javascript:void(0)" class="tablinks" onclick="openCity(event, 'ilanvideo')" <? echo ($_GET["goto"]=="videos") ? 'id="defaultOpen"' : '';?>><i class="fa fa-video-camera" aria-hidden="true"></i> <?=dil("TX458");?></a></li>
  <?if($gayarlar->dopingler_19541956 == 1){?>
  <li><a href="javascript:void(0)" class="tablinks" onclick="openCity(event, 'dopingler_19541956')" <? echo ($_GET["goto"]=="doping") ? 'id="defaultOpen"' : '';?>><i class="fa fa-rocket" aria-hidden="true"></i> <?=dil("TX548");?></a></li>
  <?}?>
  <? } // ana ilan ?>
</ul>
...
<div class="clear"></div>
<?=str_replace(array("[aylik_ilan_limit]","[ilan_resim_limit]","[ilan_yayin_sure]"),array($aylik_ilan_limit,$resim_limit,$ilan_yayin_sure." ".$periyod[$ilan_yayin_periyod]),$fonk->get_lang($snc->dil,"TX655"));?>
<div class="clear"></div>
...
<ul class="etab">
  <li><a href="javascript:void(0)" class="etablinks" onclick="openTab(event, 'etab1')" id="edefaultOpen"><?=$dilx->gosterim_adi;?></a></li>
    ...
    $dilop 		= array();
    $i			= 1;
    $sql 		= $db->query("SELECT * FROM diller_19541956 WHERE kisa_adi!='".$dilx->kisa_adi."' ORDER BY sira ASC");
...
$esnc	= $db->query("SELECT * FROM sayfalar WHERE site_id_555=000 AND ilan_no='".$snc->ilan_no."' AND dil='".$op->kisa_adi."' ");
if($esnc->rowCount()>0){
...
<table width="100%" border="0">
  ...
  <tr>
	...
	?><option <?=($snc->pbirim == $birim) ? 'selected' : '';?>><?=$birim;?></option><?
	}
	?>
	</select>
	</td>
...
<select name="emlak_durum">
		...
		foreach($parc as $val){
		?><option <?=($snc->emlak_durum == $val) ? 'selected' : '';?>><?=$val;?></option><?
		}
		?>
</select>
    </td>
...
<select name="emlak_tipi" onchange="konut_getir(this.options[this.selectedIndex].value);">
		...
		$arsa		= $parc[2];
		foreach($parc as $val){
		?><option <?=($snc->emlak_tipi == $val) ? 'selected' : '';?>><?=$val;?></option><?
		}
		?>
</select>
    </td>
...
<select id="il" name="il" onchange="ajaxHere('ajax.php?p=ilce_getir&il_id='+this.options[this.selectedIndex].value,'ilce'),$('#semt').html(''),yazdir();">
		...
		?><option value="<?=$row->id;?>" <?=($row->id == $snc->il_id) ? 'selected' : '';?>><?=$row->il_adi;?></option><?
		}
		?>
</select>
    </td>
...
<select name="ilce" id="ilce" onchange="ajaxHere('ajax.php?p=mahalle_getir&ilce_id='+this.options[this.selectedIndex].value,'semt'),yazdir();">
		...
		?><option value="<?=$row->id;?>" <?=($row->id == $snc->ilce_id) ? 'selected' : '';?>><?=$row->ilce_adi;?></option><?
		}
		}
		?>
</select>
  ...
  <tr>
	...
	<select onchange="yazdir();" name="mahalle" id="semt">
		...
		if($snc->ilce_id != 0){
			...
			?><option value="<?=$row->id;?>" <?=($snc->mahalle_id == $row->id) ? 'selected' : '';?>><?=$row->mahalle_adi;?></option><?
			}
			}
			?></optgroup><?
			}
...
<select name="konut_tipi">
		...
		foreach($parc as $val){
		?><option <?=($snc->konut_tipi == $val) ? 'selected' : '';?>><?=$val;?></option><?
		}
		?>
</select>
    </td>
...
<select name="konut_sekli">
		...
		foreach($parc as $val){
		?><option <?=($snc->konut_sekli == $val) ? 'selected' : '';?>><?=$val;?></option><?
		}
		?>
</select>
    </td>
...
<select name="bulundugu_kat">
		...
		foreach($parc as $val){
		?><option <?=($snc->bulundugu_kat == $val) ? 'selected' : '';?>><?=$val;?></option><?
		}
		?>
</select>
    </td>
...
?>
  <tr id="yapi_durum_con">
    ...
    <select name="yapi_durum">
		...
		$parc		= explode("<+>",$ypidrm);
		foreach($parc as $val){
		?><option <?=($snc->yapi_durum == $val) ? 'selected' : ''; ?>><?=$val;?></option><?
		}
		?>
</select>
    </td>
  ...
  <tr id="oda_sayisi_con">
    ...
    <td>
     <select name="oda_sayisi">
		...
		foreach($parc as $val){
		?><option <?=($snc->oda_sayisi == $val) ? 'selected' : '';?>><?=$val;?></option><?
		}
		?>
</select>
    </td>
  ...
  <tr id="isitma_con">
    ...
    <td>
     <select name="isitma">
		...
		$parc		= explode("<+>",$isitma);
		foreach($parc as $val){
		?><option <?=($snc->isitma == $val) ? 'selected' : '';?>><?=$val;?></option><?
		}
		?>
</select>
    </td>
...
<? } ?>
  ...
  <tr id="esyali_con">
    ...
    <select name="esyali">
        ...
        <option value="0" <?=($snc->esyali == 0 AND $snc->esyali != '') ? 'selected' : '';?>><?=$fonk->get_lang($snc->dil,"TX168");?></option>
</select>
    </td>
  </tr>
...
?>
   <tr id="kullanim_durum_con">
    ...
    <select name="kullanim_durum">
       <?php
		$parc		= explode("<+>",$kuldrm);
		foreach($parc as $val){
		?><option <?=($snc->kullanim_durum == $val) ? 'selected' : '';?>><?=$val;?></option><?
		}
		?>
</select>
    </td>
  ...
  <? } ?>
    ...
    <select name="site_ici">
        ...
        <option value="0" <?=($snc->site_ici == 0 AND $snc->site_ici != '') ? 'selected' : '';?>><?=$fonk->get_lang($snc->dil,"TX168");?></option>
</select>
    </td>
  </tr>
...
<tr id="aidat_con">
	...
	<tr class="arsa_icin">
			...
			foreach($parc as $val){
			?><option<?=($snc->kaks_emsal===$val) ? ' selected' : '';?>><?=$val;?></option><?
			}
			?>
			</select>
			<?php
	...
	<tr class="arsa_icin">
			...
			$parc		= explode("<+>",$gabari);
			foreach($parc as $val){
			?><option<?=($snc->gabari=== $val) ? ' selected' : '';?>><?=$val;?></option><?
			}
			?>
			</select>
			<?php
	...
	<tr class="arsa_icin">
	  ...
	  <option value=""><?=$fonk->get_lang($snc->dil,"TX264");?></option>
		...
		foreach($parc as $val){
		?><option<?=($snc->imar_durum=== $val) ? ' selected' : '';?>><?=$val;?></option><?
		}
		?>
	  </select>
	    </td>
	...
	<tr class="arsa_icin">
            ...
            foreach($parc as $val){
            ?><option<?=($snc->tapu_durumu=== $val) ? ' selected' : '';?>><?=$val;?></option><?
            }
            ?>
            </select>
            </td>
  ...
  <tr>
	  ...
	  <?php
	  foreach($exp as $row){
	  ?><option<?=($row == $snc->krediu) ? " selected" : '';?>><?=$row;?></option><?
	  }
	  ?>
	  </select>
	  </td>
  ...
  <?}?>
	...
	<?php
	foreach($exp as $row){
	?><option <?=($snc->kimden == $row) ? 'selected' : '';?>><?=$row;?></option><?
	}
	?>
	</select>
	</td>
	...
	<?php
	foreach($exp as $row){
	?><option <?=($snc->yetkis == $row) ? 'selected' : '';?>><?=$row;?></option><?
	}
	?>
	</select>
	</td>
...
<td><font color="blue"> Sözleşmeli İlanınızı, Üye Emlakçılarımız ile Paylaşmak İçin,</font>&nbsp&nbsp&nbsp<font color="black">Kutunun İçine <font color="red"><strong> 100 </font></strong><font color="black">Yazınız.
		   <input type="text" class="form-control" id="site_id_888" name="site_id_888" value="<?=$snc->site_id_888;?>" ></td>
  </tr>
    ...
    <td><font color="blue"> Sözleşmeli İlanınızı, Grubunuzdaki Emlakçılar ile Paylaşmak İçin </font>&nbsp&nbsp&nbsp<font color="black">Kutunun İçine <font color="red"><strong> GRUP KODUNU </font></strong><font color="black">Yazınız
				<input type="text" class="form-control" id="site_id_777" name="site_id_777" value="<?=$snc->site_id_777;?>" ></td>
  </tr>
</table>
...
<br/>
    ...
    <td><font color="blue">Kapalı Portföy İlanlarınızı, Üye Emlakçılarımız ile Paylaşmak için,</font><br/> <font color="black"> Kutucuğun İçine <font color="red"><strong> 200 </font></strong><font color="black">Yazınız. ( İlan Başlık Yazısının Başına </font> <font color="red"><strong> KAPALI </font></strong><font color="black">Yazmayı Unutmayınız )</font> 
				<input type="text" class="form-control" id="site_id_699" name="site_id_699" value="<?=$snc->site_id_699;?>" ></td>
				</tr>
				

     <td><?=dil("TX27323");?></td>
	<div class="form-group" style="text-align: center;">
    <td><font color="blue"> Kapalı Portföy İlanlarınızı, KİMSE İLE PAYLAŞMAK İSTEMEZSENİZ,</font><br/> <font color="black"> Kutucuğun İçine size verdiğimiz <font color="red"><strong> ŞİFREYİ </font></strong><font color="black">Yazınız.
				<input type="text" class="form-control" id="site_id_700" name="site_id_700" value="<?=$snc->site_id_700;?>" ></td>
				</tr>
...
<div class="form-group" style="text-align: center;">
    <p><font color="red"><strong> EMLAK TALEPLERİNİZİ PAYLAŞIN </strong></font><br/><font color="black"> Sıtenize girdiğiniz Müşteri Talepleriniz, PORTALLARIMIZDA, Sizin Bilgileriniz ile YAYINLANIR. </font><br/><font color="black"> Gireceğiniz Taleplerin Başlığının Başına </font><font color="red"><strong> TALEP </strong></font> <font color="black">Yazmayı unutmayınız.</font> <br/><font color="blue"> ( PAYLAŞILAN TALEPLERİ, Sizin Sitenizdede, Sizin Bilgileriniz ile Göstermemizi isterseniz, Bizi arayabilirsiniz.) </font> <br/><font color="black">Emlak Talep İlan Kutucuğunu Doldurduysanız, </font><br/><font color="red"><strong>SÖZLEŞMELİ İLAN ve KAPALI PORTFÖY KUTUCUKLARINI DOLDURMAYINIZ. </strong> </font><br/><br/>
     ...
     <td><?=dil("TX27325");?></td>
	<div class="form-group" style="text-align: center;">
    <td><font color="blue">Emlak Taleplerinizi Paylaşmak için,</font><br/> <font color="black">Kutunun İçine <font color="red"><strong> 300 </font></strong><font color="black">Yazınız. Diğer Kutucukları İşaretlemeyiniz.
				<input type="text" class="form-control" id="site_id_702" name="site_id_702" value="<?=$snc->site_id_702;?>" ></td>
				</tr>
...
<br/>
<div class="form-group" style="text-align: center;">
    <p><strong><font color="blue"> İLANINIZI PORTALLARIMIZDA DA YAYINLANMASI İÇİN </font> <br/><font color="red">İSTEDİĞİNİZ ( Portalın / Portalların ) KUTUCUĞUNU TEKRAR İŞARETLEYİNİZ. </font></strong><br/><font color="black"> ( TEKRAR İŞARETLEMEDİĞİNİZ ( Portalımızda / Portallarımızda ) İLANINIZ YAYINLANMAZ.) </font><br/><font color="black">( İLANINIZ, Portallarımızda Sizin Bilgilerinizle Yayınlanır. )</font><br/><br/>


    <!-- İZMİR EMLAK SİTESİ -->
    <div style="margin-top: 10px;">
        
        <label for="site_id_335_checkbox"><font color="black"><strong>İZMİR EMLAK SİTESİ</strong></font>&nbsp&nbsp</label>
		<input type="checkbox" id="site_id_335_checkbox" style="margin-right: 10px;" onclick="updateSiteIds()" /><br/>
		<label style="float:left;margin-right:10px;" for="target_check" class="stm-checkbox-label"></label><span style="margin-right:5px;font-size:14px;margin-top:5px;"> ( Kutucuğu İşaretlerseniz, İLANINIZ <font color="red"><strong>( izmiremlaksitesi.com.tr )</strong></font> Portalımızda Yayınlanır. )</span>
    ...
    <div style="margin-top: 10px;">
        ...
        <label for="site_id_334_<font color="black"><strong>İSTANBUL EMLAK SİTESİ</strong></font>&nbsp&nbsp</label>
		<input type="checkbox" id="site_id_334_checkbox" style="margin-right: 10px;" onclick="updateSiteIds()" /><br/>
		<label style="float:left;margin-right:10px;" for="target_check" class="stm-checkbox-label"></label><span style="margin-right:5px;font-size:14px;margin-top:5px;"> ( Kutucuğu İşaretlerseniz, İLANINIZ <font color="red"><strong>( istanbulemlaksitesi.com.tr )</strong></font> Portalımızda Yayınlanır. )</span>
    ...
    <div style="margin-top: 10px;">
        ...
        <label for="site_id_306_checkbox"><font color="black"><strong>ANKARA EMLAK SİTESİ</strong></font>&nbsp&nbsp</label>
		  <input type="checkbox" id="site_id_306_checkbox" style="margin-right: 10px;" onclick="updateSiteIds()" /><br/>
		<label style="float:left;margin-right:10px;" for="target_check" class="stm-checkbox-label"></label><span style="margin-right:5px;font-size:14px;margin-top:5px;"> ( Kutucuğu İşaretlerseniz, İLANINIZ <font color="red"><strong>( ankaraemlaksitesi.com.tr )</strong></font> Portalımızda Yayınlanır. )</span>
...
?>
	...
	<a style="z-index:33;" id="ilandondur"  title="Döndür" class="ilanfotosil" href='javascript:window.open("<?=SITE_URL."rotate/".$row->id;?>", "mywindow","status=1,toolbar=0,resizable=1,width="+window.innerWidth+",height="+window.innerHeight+100+"").moveTo(0, 0);'><i class="fa fa-repeat"></i></a>
...
<?if($gayarlar->dopingler_19541956==1){?>
<div id="dopingler_19541956" class="tabcontent">
<?php
...
<table class="dopingtable" width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  foreach($dopingler_19541956 as $row){
  ?>
  <tr>
...
//odeme_yontemi 		= $("#odeme_yontemi").prop("checked");

if(odeme_yontemi == undefined || odeme_yontemi == ''){
//if(!odeme_yontemi){
console.log(odeme_yontemi);
...
<div class="clear"></div>
<? }elseif($odeme == "havale_eft" && $_SESSION["custom"] != ''){ // Ödeme Banka Havale/EFT ile ise... ?>
...
<?php }elseif($odeme == "iyzico" && $_SESSION["custom"] != ''){ // Ödeme Banka Havale/EFT ile ise... ?>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  $docount		= $dopingler_19541956->rowCount();
  $aco			= 0;
  while($row	= $dopingler_19541956->fetch(PDO::FETCH_OBJ)){
	## Eğer aynı dopingten varsa ve süresi bitmemişse... ##
	$isdoping	= $db->prepare("SELECT id,durum FROM dopingler_19541956 WHERE ilan_id=? AND did=? AND btarih > NOW()");
...
$("#dopingler_19541956 table tr td input:checked").each(function(index){
...
stbutton.html(stinn);
	...
	$(document).ready(function(){
	var select = 
"#yetki_bilgisi_con,#brut_metrekare_con,#site_id_888_con,#site_id_777_con,#site_id_700_con,#site_id_701_con,#site_id_661_con,#site_id_662_con,#site_id_663_con,#site_id_664_con,#site_id_665_con,#site_id_666_con,#site_id_667_con,#site_id_668_con,#site_id_669_con,#site_id_335_con,#site_id_334_con,#site_id_306_con,#konut_sekli_con,#konut_tipi_con,#bulundugu_kat_con,#yapi_durum_con,#oda_sayisi_con,#bina_yasi_con,#bina_kat_sayisi_con,#isitma_con,#banyo_sayisi_con,#esyali_con,#notu_con,#kullanim_durum_con,#site_ici_con,#aidat_con";

	<? echo ($snc->emlak_tipi == $arsa) ? '$(".tipi_konut").slideUp(500); $(select).slideUp(500);' : ''; ?>
	<? echo ($snc->emlak_tipi != $arsa) ? '$(".tipi_arsa,.arsa_icin").hide(1);' : ''; ?>
