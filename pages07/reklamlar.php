                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="btn-toolbar" role="toolbar">
                                        ...
                                        <div class="btn-group">
										<button type="button" class="btn btn-info waves-effect waves-light w-lg m-b-5" onclick="window.location.href='index.php?p=reklam_ekle';"> <i class="fa fa-plus"></i> Yeni Ekle</button>
...
if($sil != ""){
$db->query("DELETE FROM reklamlar_19561954 WHERE id=".$sil." ");
...
$id			= $gvn->rakam($id);
if($action == 'sil'){
$db->query("DELETE FROM reklamlar_19561954 WHERE id=".$id);
}
} // FOREACH END
} // eğer varsa
...
?>
										...
										<tbody>
										<?php
										$sorgu		= $db->query("SELECT * FROM reklamlar_19561954 ORDER BY id DESC LIMIT 0,500");
										...
										<label for="checkbox<?=$row->id;?>"></label>
										 ...
										 <td><?=$row->baslik;?></td>
										 <td><?=$reklam_alanlari[$row->tipi];?></td>
										 <td><?=date("d.m.Y H:i",strtotime($row->tarih));?></td>
										 <td><?=($row->suresiz==1) ? '<strong>Süresiz</strong>' : date("d.m.Y",strtotime($row->btarih));?></td>
										 <td>
                                        ...
                                        <ul class="dropdown-menu" role="menu">
                                            <li><a href="index.php?p=reklam_duzenle&id=<?=$row->id;?>">Düzenle</a></li>
                                            <li><a href="index.php?p=reklamlar&sil=<?=$row->id;?>">Sil</a></li>
                                        </ul>
                                    </div>
