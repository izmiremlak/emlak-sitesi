...
if($fonk->bosluk_kontrol($tcno) == true AND $gayarlar->tcnod == 1 && $hesap->tcno == ''){
die('<span class="error">'.dil("TX369").'</span>');
}elseif($gayarlar->tcnod == 1 AND $gvn->tcNoCheck($tcno) == false && $hesap->tcno == ''){
...
$kontrol2		= $db->prepare("SELECT * FROM hesaplar WHERE site_id_555=999 AND durum=1 AND id!=".$hesap->id." AND (email=? OR ip=?) ");
...
$kontrol		= $db->prepare("SELECT * FROM hesaplar WHERE site_id_555=999 AND (id!=".$hesap->id.") AND email=?");
...
if($telefon != ''){
$kontrol3		= $db->prepare("SELECT * FROM hesaplar WHERE site_id_555=999 AND (id!=".$hesap->id.") AND telefon=?");
...
$gnc			= $db->prepare("UPDATE hesaplar SET parola=? WHERE site_id_555=999 AND id=".$hesap->id." ");
...
//adi=?,soyadi=?,
#$adi,$soyadi,
$sql		= $db->prepare("UPDATE hesaplar SET telefon=?,email=?,sms_izin=?,mail_izin=?,sabit_telefon=?,telefond=?,sabittelefond=?,epostad=?,avatard=?,tcno=?,unvan=?,vergi_no=?,vergi_dairesi=?,adres=?,tconay=?,nick_adi=?,hakkinda=? WHERE site_id_555=999 AND id=".$hesap->id);
$sql->execute(array($telefon,$email,$sms_izin,$mail_izin,$sabit_telefon,$telefond,$sabittelefond,$epostad,$avatard,$tcno,$unvan,$vergi_no,$vergi_dairesi,$adres,$tconay,$nick_adi,$hakkinda));
