...
$yfotolar		= $db->query("SELECT * FROM galeri_foto WHERE site_id_555=999 AND sayfa_id=".$snc->id." AND dil='".$dil."' ORDER BY id DESC");
...
if(!empty($tabs)){
	foreach($tabs AS $key=>$val){
		...
		if($dilop->rowCount()>0 && $fonk->bosluk_kontrol($val['baslik'])==false && ($multi->rowCount()>0 && $snc->id == $multif->id)){
			...
			$kontrol	= $db->prepare("SELECT id,ilan_no FROM sayfalar WHERE site_id_555=000 AND ilan_no=? AND dil=?");
			...
			if($kontrol->rowCount()>0){ // var ise
				...
				try {
 				...
 				$prepare = "INSERT INTO sayfalar SET site_id_555=555,site_id_450=450,site_id_444=444,site_id_333=333,site_id_222=222,site_id_111=111,baslik=?,fiyat=?,ilan_no=?,emlak_durum=?,emlak_tipi=?,ulke_id=?,il_id=?,ilce_id=?,mahalle_id=?,konut_tipi=?,konut_sekli=?,bulundugu_kat=?,metrekare=?,brut_metrekare=?,yapi_durum=?,oda_sayisi=?,bina_yasi=?,bina_kat_sayisi=?,isitma=?,banyo_sayisi=?,esyali=?,kullanim_durum=?,site_ici=?,aidat=?,cephe_ozellikler=?,ic_ozellikler=?,dis_ozellikler=?,maps=?,resim=?,acid=?,tipi=?,dil=?,url=?,tarih=?,gtarih=?,pbirim=?,icerik=?,metrekare_fiyat=?,ada_no=?,parsel_no=?,pafta_no=?,kaks_emsal=?,gabari=?,tapu_durumu=?,katk=?,krediu=?,takas=?,kimden=?,yetkis=?,yetki_bilgisi=?,site_id_888=?,site_id_777=?,site_id_699=?,site_id_700=?,site_id_701=?,site_id_702=?,site_id_661=?,site_id_662=?,site_id_663=?,site_id_664=?,site_id_665=?,site_id_666=?,site_id_667=?,site_id_668=?,site_id_669=?,site_id_335=?,site_id_334=?,site_id_306=?,altyapi_ozellikler=?,konum_ozellikler=?,genel_ozellikler=?,manzara_ozellikler=?,notu=?,durum=?,keywords=?,description=?,btarih=?,pid=? ";
